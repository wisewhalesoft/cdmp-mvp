...
>
> 版本：v1.1 / 2026-08-13（**職稱來源更正為 `title_name` / `titleName`**，見 B.1.1）
...
### B.1.1 職稱來源更正（F116 spec v1.1.1 / 2026-08-13）

使用者實機檢視後更正職稱來源：**`ob_emphire.jfun_nm` → `ob_emphire.title_name`；API 欄位 `jfunNm` → `titleName`**（已核可命名）。
理由：dev MSSQL 317 筆實測中 `jfun_nm` 有 **91%（289/317）為「營業一般職」**，屬職能分類而非職稱，
放進樞紐列標籤幾乎無辨識度；`title_name` 才是真正職稱。上游 US-182 與 F116 spec v1.1.1 之 BR-5 已反轉。

原型與本附錄同步更新之站點：示範資料欄位 `jfun` → `titleName`（8 處）、斷言掛點 `data-pv-jfunnm` → `data-pv-titlename`、
`pvEmpLabel()` 取值與相關註解、B.2 D-3、B.5 Phase B-1 兩張表、B.9 驗證條目。原型內已無 `jfun` 字樣。

**連帶的版面影響**：`title_name` 最長值為「業務專員(電銷)」（8 字），較舊來源長，列標籤欄由 278px 增為 311px。
處理方式見 B.2 D-3a（不截斷、不設欄寬上限）與 B.9 第 3a 項（四種寬度零溢出實測）。
D-8 之「凍結偏移量由 JS 量測」即為此變更未造成兩欄重疊的原因。
...
| D-3 | `titleName = null` 之呈現 | **完全省略職稱區段與其前方分隔符號，不留空位** | AC-8 允許「空白」；保留破折號會產生「20501 - 王大明 -」的斷尾，讀起來像資料損毀；且 `-` 在本表已被指派為「無分派案件」的語意（值格），重複使用會造成歧義 |
| D-3a | 職稱長度上限與截斷策略（v1.1.1） | **不截斷、不設欄寬上限**，列標籤欄維持依內容自動撐開 | 真實 `title_name` 最長值為「業務專員(電銷)」（8 字），實測列標籤欄 311px（1280/1366/1440）／319px（1920），四種寬度下**儲存格零溢出**。截斷職稱會把「業務專員(電銷)」削成「業務專員(電…」，正好抹除 v1.1.1 換掉 `jfun_nm` 所要取得的辨識度。欄位變寬後名單代號欄可見數由 7 降為 6（@1280），仍在既有「12+ 欄本就需橫捲」的設計前提內，且總計欄凍結確保橫捲時不會失去列總計 |
...
| D-8 | 凍結偏移量的取得方式 | 每次 render 後由 `pvSyncStickyOffset()` 量測列標籤欄實寬寫入 CSS 變數 `--pv-first-w`（fallback `200px`），並在切換頁籤／視窗 resize／字型載入完成時重測 | 列標籤欄寬度隨職稱長度與新人標註而變（實測 311px @1280/1366/1440、319px @1920；v1.1.1 職稱改用 `title_name` 後由 278px 增為 311px），寫死偏移量會造成兩欄重疊——**此決策正是 v1.1.1 換欄位後版面未擠壞的原因**；`#panel-pivot` 初始為 `hidden`（量得 0），故必須在 `switchSnapTab('pivot')` 時補測 |
...
| 職稱 | `text-gray-500`；null → **整段省略** | `[data-pv-titlename]`（null 時元素不存在） |
| 新人 | 琥珀 pill ＋ `user-plus` ＋「新人」 | `[data-pv-newcomer="true"]`（false 時元素不存在） |

> **職稱來源（v1.1.1 更正）**：`ob_emphire.title_name` → API `titleName`。**不是** `jfun_nm` / `jfunNm`——
> dev MSSQL 317 筆實測顯示 `jfun_nm` 有 91%（289/317）為「營業一般職」，屬職能分類、放進列標籤無辨識度。
> 原型示範資料之 `titleName` 值均取自真實分佈，勿改回舊來源或替換為虛構職稱。

**示範資料邊界覆蓋**（`PIVOT_DEPTS`，職稱值取自 dev MSSQL `title_name` 真實分佈）：

| 員編 | 姓名 | `titleName` | `isNewcomer` | 示範之邊界 |
|---|---|---|---|---|
| `20501` | 王大明 | `null` | `false` | 職稱缺值 → 省略職稱與分隔符 |
| `21557` | 林淑芬 | 業務專員(電銷) | `true` | **最長職稱（8 字）＋ 新人標註 = 版面最寬組合** |
| `22159` | 陳志明 | 業務專員 | `false` | 最常見職稱（126 筆）之基準樣態 |
| `20988` | 張美玲 | 業務襄理 | `false` | — |
| `21744` | 李國華 | 業務副理 | `false` | — |
| `30112` | 黃雅婷 | `null` | `true` | 兩個邊界同時發生（省略職稱後 pill 不留空位） |
| `40233` | 吳建宏 | 業務專員(電銷) | `false` | 最長職稱、無標註 |
...
3. **身分字串**：`21557` 全文為 `21557 - 林淑芬 - 業務專員(電銷) 新人`（2 個分隔符、**單行不換行**）；`20501` 無 `[data-pv-titlename]` 且僅 1 個分隔符；`22159` / `30112` 之新人 pill 文字為「新人」；部門列／其他列／`(空白)` 皆無職稱與標註元素。頁面全域無 `jfun` 字樣殘留。
3a. **最長職稱版面**：`21557`（最長職稱 ＋ 新人 pill）與其他所有列標籤格 `scrollWidth <= clientWidth`（零溢出），於 1280 / 1366 / 1440 / 1920 四種寬度皆成立。
...
# 附錄 D — F120 Stage 0 試算頁「名單基礎預估數量總覽」（US-184）

> **狀態：🔴 Draft，待人工審閱閘。** 本附錄涵蓋 [F120 v1.1](specs/features/F120-stage0-list-estimate-overview.md)（US-184 v1.1）之 UI/UX 設計，
> 並就 spec §13.2 交付本角色之 **OQ-F120-U1 / U2 / U3** 三項提出定案。
> 架構決策見 [AD-E07-51](specs/implementation-log/AD-E07-51-f120-list-estimate-overview.md)。
> 本輪**不新增頁面、不新增路由、不改動任何原型的側邊欄或頁面階層**；只在既有 Stage 0 試算頁新增第三個區塊。

## D.1 Context

Stage 0 試算頁（`prototypes/30-stage0-estimate.html`）現有兩個區塊都是**部門維度**：
「部門負載總覽」（部門 × 月統計）與「部門每日分派明細」（部門 × 日月曆）。
業務部長在觸發月名單分派前，缺一個「**本月到底跑哪幾份名單、量體怎麼分佈在產品類別之間**」的視角，
必須逐一切換名單篩選器才能核對，容易漏看漏設 / 誤設 / 量體異常的名單。

F120 在同一頁新增第三個區塊，以**名單**為單位列出 名單編號 / 名單名稱 / 篩選條件 / 預估數量，
並依產品類別分為五組（汽車 → 機車 → 一般商品 → 多重產品類別 → 未分類）小計。
數值底座與既有兩區塊**同一份**（名單層總量），因此本區塊的「預估數量總計」與部門區塊的「本月全名單總量」是同一個數字。

| 檔案 | 本輪變更 |
|---|---|
| `prototypes/30-stage0-estimate.html` | 新增第三個區塊；示範名單母體改為帶篩選條件的資料結構；新增 4 個示範場景；處長唯讀提示補一句範圍界定 |
| `docs/ui-ux-design-overview.md` | 本附錄 |

**未改動**：既有兩個區塊的表格結構、欄位、排序、捨入、缺口、人均、處長 scope 隔離行為；`orgMonthTotal` 之算式；其餘任何原型檔。

## D.2 設計決策

| # | 決策 | 選擇 | 理由 / AC 依據 |
|---|---|---|---|
| **D-1** | **區塊視覺順序（OQ-F120-U1 ③）** | 既有兩區塊**之後**、頁尾提示**之前**（AC-LIST-01 之預設位置，本輪確認不動） | 頁面本來的敘事是「總量 → 部門 → 每日」，由粗到細；本區塊是對**同一個總量**換一個切法（依名單、依產品類別），屬於「橫向拆解」而非上游，接在部門區塊之後讀起來是「這個總量是由哪些名單組成的」。若移到最上方，會把頁面主題（每日工作量可行性）降級 |
| **D-2** | **分組是否可摺疊（OQ-F120-U1 ③）** | **可摺疊，預設全部展開**；另有「全部收合／全部展開」一鍵切換 | 本區塊的用途是抓漏設 / 誤設，預設就必須看得到名單列；收合是「已經看完這組」之後的收納動作。11 張名單時表格約 18 列，摺疊讓使用者能只留下正在查的那組 |
| **D-3** | **分組標題列 ＝ 小計列（不另拉一列小計）** | 名單數 / 預估數量小計 / 佔比**直接放在分組標題列**的對應欄位 | ①收合分組時**數字永遠不會被藏起來**——若小計另立一列放在名單列之後，收合就會連小計一起收掉，或必須做例外處理②小計與它所加總的欄位天然對齊，不需要額外的欄位標籤③少一列＝少一層視覺噪音。AC-LIST-08 只要求「顯示三項」，未要求獨立列 |
| **D-4** | **「篩選條件」多筆條件的串接（OQ-F120-U1 ①）** | **每一筆條件各自一個標籤（chip）**，不用任何分隔字元串成一句 | 一份名單的多筆條件是 **AND**；用「、」或「，」串接會被讀成「或」。分開成標籤，加上區塊底部一句「多個條件為『且』的關係（需同時成立）」，語意才不會被格式誤導。每個標籤的文字一律由 `formatConditionSummary()` 產生（AC-LIST-04 / F119 BR-10），本區塊**沒有**任何自拼字串的路徑 |
| **D-5** | **截斷門檻與展開互動（OQ-F120-U1 ①）** | 預設顯示**前 2 筆** ＋ 真正的 `<button>`「＋N 項」，點擊**就地展開該列**（不是 hover、不是 tooltip、不是彈出層）；單一標籤過長時以 `max-w-[18rem] truncate` ＋ `title` 保全語意 | ①門檻 **2** 沿用名單定義列表卡片既有的「前 2 筆 ＋N」慣例，全站只有一種截斷語彙②**不用 hover**：鍵盤與觸控走不到，且 AC-LIST-04 要求「能呈現全部條件」，hover 不算能呈現③**不用 tooltip**：內容不能選取複製、螢幕閱讀器不友善、在表格內容易被裁切④**不用彈出層**：表格容器有 `overflow-x-auto`，浮層會被裁掉⑤就地展開讓預設列高維持一致（實測 47px），表格仍可快速掃描，展開後焦點回到同一顆按鈕 |
| **D-6** | **處長專屬語意標示的文案與位置（OQ-F120-U1 ②，AC-LIST-11 必要條件）** | **三個觸點**：①區塊標題旁藍色 `全公司口徑` 徽章②區塊內、表格上方的藍色說明條（第一行逐字採 spec 允許用語：「**本區塊為全公司名單層總量，非您所屬轄區之分派量**」；第二行點名對比：「上方兩個部門區塊只顯示您的轄區部門（北區電銷一課）；本區塊列出的是本月全公司所有啟用名單，兩者口徑不同，請勿相加或直接比較。」）③**總計列**的「總計」後綴「（全公司口徑）」 | ①說明條放在**區塊內**而非頁首——頁首已有一則紫色「唯讀模式／轄區」提示，把校正訊息放在它旁邊等於要使用者自己比對兩段話；放在資料正上方，因果最短②**第三個觸點是刻意的**：業務主管常直接跳到最下面看總計，說明條這時已捲出畫面，而**總計正是最容易被誤讀成自己轄區工作量的那個數字**③用藍（資訊）而非琥珀 / 紅——這不是錯誤或警告，是口徑說明；也刻意不用頁面既有的紫（唯讀／處長）與青（轄區），避免把「非轄區」的訊息塗上「轄區」的顏色 |
| **D-7** | **頁首處長提示補一句範圍界定**（既有 `#readOnlyBanner` 文案微調） | 標題由「僅顯示您轄區部門的預估資料」改為「**部門相關區塊**僅顯示您轄區部門的預估資料」，說明句補「頁面最下方的『名單基礎預估數量總覽』則為全公司名單層總量，不受轄區限制」 | 不改這句，同一頁會出現正面矛盾（頁首說「本頁只有轄區」、區塊說「本區塊是全公司」），而矛盾會直接抵消 AC-LIST-11 想達成的防呆。**僅改文字，未改該提示的版面、顏色、顯示條件**，亦未觸及兩個既有區塊 |
| **D-8** | **單一名單鑽探模式的佔比（OQ-F120-U2）** | **保留佔比欄位**（不抽欄）、所有佔比格顯示「**—**」，並在欄位標題下方加一行 11px 灰字「單一名單檢視不計算佔比」；區塊底部說明同步替換為「目前為單一名單檢視，佔比不具比較意義，故不計算；改回『全部名單（彙總）』即會顯示各組佔比」 | ①**不顯示 100%**：單一名單時佔比恆為 100%，那是同義反覆而非資訊，而且會被讀成「這張名單佔全月總量的 100%」——一個明確錯誤的結論②**不抽掉欄位**：欄數隨模式改變會讓表頭在兩種模式下不一致，破壞欄位位置的肌肉記憶，也讓下游 React／測試多一種版面分支③「—」在本頁既有語彙就是「不適用 / 無法計算」（人均 —、缺口 —），且 AC-LIST-08 已規定總計為 0 時佔比顯示「—」；沿用同一個符號＋就地說明，使用者不需要學第二個記號 |
| **D-9** | **無估算值名單的視覺標記（OQ-F120-U3）** | **完全沿用**本頁既有的逾時語彙，不新增任何顏色或圖示：琥珀 `bg-amber-100 / text-amber-800 / border-amber-300` ＋ `hourglass`。三個層級：**列**＝「—」＋`未能估算`徽章（`title` 為「本次未納入合計」）＋ 整列 `bg-amber-50/40`；**分組**＝分組標題列上「本組合計未涵蓋 N 張未能估算的名單」徽章；**區塊**＝標題與總計數字旁 `不完整` 徽章（與既有 KPI 卡「不完整」徽章同一組 class） | 全頁只能有一套「這個數字不完整」的講法。逐筆名單與後端原文訊息仍**只**由頁首既有的琥珀 banner 承載（不重複列一次），本區塊只做「這一列 / 這一組 / 這個總計受影響」的就地指認，並在區塊底部指回上方提示。**刻意不用**紅色與 `alert-triangle`——本頁紅＝人均超載、`alert-triangle`＝資料池偏低，借用會製造第二種解讀 |
| **D-10** | 分組標題的顏色語意 | 三個**已登錄**產品類別沿用既有 `prodKindBadge()` 色彩（01 藍 `car` / 02 翠綠 `bike` / 03 紫 `package`）；**未登錄代碼**灰 ＋ `alert-circle`（同 `prodKindBadge()` 的 fallback）；兩個合成分組灰 ＋ 各自圖示（多重產品類別 `layers`、未分類 `help-circle`）＋ 一句灰字說明 | 顏色只保留給「真的是某個產品類別」的分組，合成分組不該假裝自己是一種商品。三個灰色分組靠**圖示 ＋ 名稱 ＋ 說明句**區分，不必為此新增色票。分組標題文字一律取自可選值清單（`PROD_KIND_OPTIONS`），未硬編碼中文字串（AC-LIST-05） |
| **D-11** | 合成分組鍵不使用可讀英文 | 內部鍵用 `combined` / `unset`，DOM 掛點用 `data-group-kind="code｜combined｜unset"` ＋ `data-group-id="g-01 / g-combined / g-unset"` | AC-LIST-14 / §9.1 把 `MULTI`、`UNCLASSIFIED` 列入黑名單。雖然黑名單針對的是使用者可見文字，但只要這兩個字串完全不進 DOM，「全文掃描」用哪種寫法都不會誤判 |
| **D-12** | 分組判定寫成純函式 `resolveListGroup(payload)` | 逐條對應 spec §5.2 GROUP-RESOLVE 的 6 個步驟；運算子預設值一律經 `resolveCategoricalOperator()` | 全函式 ⇒ 每張名單恰好落在一組（`I-F120-01` 互斥且完備）是**結構上的必然**，不靠測試維持。原型刻意不寫 `cond.operator` 加預設值的第二個落點，維持 F119 BR-11 的單一 fallback |
| **D-13** | 條件字串**不去重**顯示 | 可選值 `['01','01']` 顯示為「產品類別：汽車、汽車」 | 去重只發生在**分組判定**（§5.2 步驟 6b）；顯示端必須是 `formatConditionSummary()` 的逐字輸出，否則與名單詳情不一致（TC-184-12）。看起來像瑕疵但這是刻意的——見 D.7 之 **D-Q2** |
| **D-14** | 處長無轄區（`scope = null`）時本區塊的處置 | 本區塊**照常完整呈現**，由 `placeListOverview()` 搬到降級卡片下方 | F120 §10.2 明列「處長 scope 為 null → 本區塊完整回傳（不降級）」。名單層總量不依轄區，轄區辨識失敗不影響其可算性；若跟著整頁降級，等於把一個算得出來的答案藏起來 |
| **D-15** | 示範資料策略 | 既有 3 筆名單母體與其件數**完全不動**（維持所有既有場景的數字不變）；另備一份 11 筆的完整分組母體，掛在新的示範場景按鈕上 | 直接把母體改大會讓既有 6 個場景的數字全部位移，F049 既有的人工審閱結論與可能的下游斷言都會失效。分成兩份母體，預設路徑零擾動，完整分組另開一鍵 |
| **D-16** | 佔比示範數字刻意加總為 **99%** | 完整分組母體的各組小計四捨五入後為 34+16+6+2+12+29 = 99% | AC-LIST-08 明訂「加總可能不等於 100%，屬預期行為，不強制湊整」。若示範資料剛好湊成 100%，這條規則在原型上就無從驗證，人工審閱與下游 ring 都會建立在錯誤前提上 |

## D.3 色彩系統（本輪未新增色票，僅新增語意）

| 語意 | 色票 | 用途 | 與既有元素之區隔 |
|---|---|---|---|
| 產品類別（已登錄） | `bg-blue-100/text-blue-700`（汽車）、`bg-emerald-100/text-emerald-700`（機車）、`bg-violet-100/text-violet-700`（一般商品） | 分組標題徽章 | 逐字沿用 `28-scoring-config.html` 之 `prodKindBadge()`，全站產品類別同一組顏色 |
| 產品類別（未登錄代碼） | `bg-gray-100/text-gray-500` ＋ `alert-circle` | 未登錄代碼分組標題 | 同 `prodKindBadge()` 之 fallback；灰＝「無法對照」 |
| 合成分組 | `bg-gray-100/text-gray-600` ＋ `layers`／`help-circle` | 多重產品類別／未分類 | 不給顏色——它們不是某個商品；靠圖示與說明句區分彼此 |
| 無估算值（列 / 組 / 區塊） | `bg-amber-100 / text-amber-800 / border-amber-300` ＋ `hourglass`；列底 `bg-amber-50/40` | 三個層級的標記 | **與頁首既有逾時 banner 與 KPI「不完整」徽章同一套**，全頁只有一種逾時視覺（OQ-F120-U3） |
| 處長口徑標示 | `bg-blue-50 / border-blue-200 / text-blue-900` ＋ `globe`；徽章 `bg-blue-100/text-blue-800` | 區塊內說明條、標題徽章、總計後綴 | 藍＝資訊（非錯誤）；刻意避開頁首的紫（唯讀／處長）與青（轄區），以免把「非轄區」訊息塗上轄區色 |
| 條件標籤 | `bg-gray-50 / text-gray-600 / border-gray-200` | 篩選條件 chip | 中性；「＋N 項」按鈕用 `bg-blue-50/text-primary/border-blue-200`，讓「可點」與「純顯示」一眼分得開 |

> 對比度：`text-amber-800 #92400E` on `#FEF3C7` ≈ 6.4:1、`text-blue-900 #1E3A8A` on `#EFF6FF` ≈ 12.6:1、`text-gray-600 #4B5563` on `#F9FAFB` ≈ 8.9:1、`text-primary #2563EB` on `#EFF6FF` ≈ 7.0:1 —— 均達 WCAG 2.1 AA。灰字說明（`text-gray-400` on white ≈ 2.8:1）僅用於**輔助性重述**（分組說明句），其資訊在分組名稱與底部說明中皆有非灰色的等義呈現，不作為唯一傳達管道。

## D.4 檔案結構

```
prototypes/
└── 30-stage0-estimate.html   # Stage 0 試算 —— 新增第三個區塊「名單基礎預估數量總覽」
```

**未新增原型檔**：F120 是既有頁面的加性擴充（同一路由 `/assignment/estimate`、同一側邊欄項目），故直接就地修改 ground truth 檔。
...
| 項目 | 內容 |
|---|---|
| 卡片外殼 | 沿用本頁兩個既有區塊的形制：`bg-white rounded-xl border border-border overflow-hidden` ＋ `px-5 py-3.5` 標題列 ＋ lucide 圖示 ＋ `text-sm font-semibold` 標題 ＋ 灰字提示 |
| 位置 | `#listOverviewAnchor`，位於「部門每日分派明細」之後、頁尾藍色提示之前 |
| 名單母體 | `LIST_UNIVERSE_BASE`（3 筆，既有數字不變）／ `LIST_UNIVERSE_RICH`（11 筆，覆蓋 §10.2 全部邊界案例）；每筆帶篩選條件內容 |
| 名單集合 | `listOverviewRows()`：與部門區塊同一份母體、同一個名單篩選器，但**不排除**未能估算的名單（AC-LIST-10） |
| 條件格式化 | 自 `27-list-definition.html` **逐字複製** `formatConditionSummary()` 及其相依區段（AC-LIST-04 / F119 BR-10），並在原型內標註「修改此段必須四份原型同步」 |
| 分組解碼來源 | `PROD_KIND_OPTIONS`（＝可選值清單）同時供給**分組標題**與**條件字串的值標籤**，畫面上不會出現「分組叫汽車、條件卻叫別的名字」 |

### Phase D-2：表格層級

| 層級 | DOM | 內容 |
|---|---|---|
| 表頭 | `<thead>` 一列 | 名單編號 / 名單名稱 / 篩選條件 / 名單數 / 預估數量 / 佔比（單一名單模式時佔比欄多一行灰字說明） |
| 分組列（＝小計列） | `<tbody data-testid="list-overview-group">` → `<tr data-testid="list-overview-group-row">`，首格為 `<th colspan="3" scope="colgroup">` | 摺疊按鈕（`<button>` ＋ `aria-expanded` ＋ `aria-controls`）＋ 分組徽章 ＋ 說明句 ＋（有未能估算時）琥珀徽章；右三欄＝名單數 / 預估數量小計 / 佔比 |
| 名單列容器 | `<tbody id="lo-body-{gid}">`（收合時加 `hidden`） | 每組一個，與分組列分開，避免巢狀 `tbody` |
| 名單列 | `<tr data-testid="list-overview-list-row" data-list-no>` | 名單編號（等寬、縮排 `pl-11`）／名單名稱／條件標籤群／（名單數欄留白）／預估數量或「—」＋徽章／（佔比欄留白） |
| 總計列 | `<tfoot>` → `<tr data-testid="list-overview-total-row">` | 「總計」（處長時後綴「（全公司口徑）」）＋「名單總數」＋「預估數量總計」（含小標籤，確保 §9.2 用語逐字出現於 DOM）＋ 佔比「—」 |
| 區塊底部說明 | 表格外 `div` | 分組列／總計列的讀法 ＋ 條件為「且」的關係 ＋ 佔比公式與「可能不等於 100%」 ＋（有未能估算時）指回頁首提示 |

### Phase D-3：示範場景（新增 4 顆，既有 7 顆未動）

| 按鈕 | 場景 | 驗證的 AC |
|---|---|---|
| 名單分組完整示範 | 11 筆母體，六個分組（三個已登錄代碼＋未登錄代碼 `09`＋多重產品類別＋未分類） | AC-LIST-05 / 06 / 06a / 07 / 08（含 99% 加總） |
| 分組含未能估算名單 | 同上 ＋ 2 張名單未能估算，其中 1 張是「機車」組的唯一名單 → 該組名單數 1、小計 0、仍顯示 | AC-LIST-10 ＋ §12 G-6（依名單數隱藏、非依小計） |
| 名單全數未能估算 | 11 張全部未能估算 → 預估數量總計 0、全部佔比「—」 | AC-LIST-08 末段 / TC-184-15 |
| 當月無啟用名單 | 母體為空 → 空狀態，不渲染分組 / 名單列 / 小計 / 總計 | AC-LIST-12 |

> 處長視角以右上角**既有**角色選單切換（可與上述任一場景組合）；單一名單鑽探以**既有**「名單篩選」下拉切換。本區塊未新增任何篩選控制項（AC-LIST-02）。

## D.6 共用 UI 模式（本輪新增／沿用）

| 模式 | 規則 |
|---|---|
| 群組表格的小計位置 | 小計放在**分組標題列**，不另立小計列 → 收合分組永遠不會把數字藏起來 |
| 長尾清單儲存格 | 前 N 筆 ＋ 真正的 `<button>`「＋N 項」就地展開；不用 hover / tooltip / 浮層；重繪後把焦點還給同一顆按鈕 |
| 「不適用」記號 | 一律「—」，與「0」嚴格區分；同一頁只用這一個符號表示不適用 / 無法計算 |
| 降級語彙唯一化 | 同一種降級（逾時 / 未能估算）在同一頁只能有一套顏色＋圖示＋措辭；逐筆清單只由一個地方承載，其餘位置只做就地指認並指回該處 |
| 角色口徑校正 | 當某區塊的資料範圍與同頁其他區塊**相反**時，校正訊息要貼著資料本身，並在「最容易被誤讀的那個數字」旁再放一次 |

## D.7 待人工確認 / 給上游的問題

| ID | 問題 | 我的處置 | 需要誰裁決 |
|---|---|---|---|
| **D-Q1** | `27-list-definition.html` 的示範可選值標籤與正式 seed **不一致**（該檔寫「汽車新車」「其他商品」；`pooldata-field-option.json` 與 `28-scoring-config.html` 都是「汽車」「一般商品」）。TC-184-12 要求本區塊的條件字串與名單詳情**逐字元相同** | 本區塊採 **seed 值**（汽車 / 機車 / 一般商品），與 F120 分組標題定案一致；未改 27（不在本輪授權範圍） | team lead：是否指派人修正 27 的示範標籤。React 端兩處共用同一個解碼器，故此落差只存在於原型層，但會讓「拿兩份原型逐字比對」的驗證方式失準 |
| **D-Q2** | spec 未規定「條件字串是否要對重複的可選值去重顯示」。目前顯示為「產品類別：汽車、汽車」 | **不去重**——去重只屬於分組判定（§5.2 步驟 6b）；顯示端一去重就與名單詳情不一致，違反 AC-LIST-04 的單一格式化來源 | spec-writer：若同意，建議在 AC-LIST-04 補一句「顯示端不做去重」，避免下游把它當缺陷修掉 |
| **D-Q3** | 分組全數未能估算、但**總計 > 0** 時，該組佔比為 0 除以總計 = **0%**（非「—」） | 依 §10.2「佔比『—』或 0%（依總計是否為 0）」顯示 **0%**，並在同一列以琥珀徽章說明「本組合計未涵蓋 N 張未能估算的名單」 | 確認即可，非爭議項。列此項是因為「0%」與 AC-LIST-08 的「不得顯示 0%」字面相近，怕下游誤修 |
| **D-Q4** | F049 自身的**頁面層**空狀態（AC-AGG-3「本月尚無啟用名單」）在本原型**從未實作**——當月無名單時，KPI 仍顯示 0、部門表與月曆仍渲染整月 0 件 | 本輪只實作 F120 區塊自己的空狀態（AC-LIST-12），未動 F049 區塊 | team lead：這是 F120 之前就存在的落差，需不需要另開修補項 |
| **D-Q5** | 為了讓兩個既有部門區塊與本區塊共用同一份名單集合（AC-LIST-02），示範資料的「名單篩選」下拉改為**由名單母體動態產生**（原本是寫死的 3 個 `<option>`） | 預設母體下產生的選項文字與原本**逐字相同**（`{名稱}（{編號}）`），視覺零差異 | 告知性。若下游有針對該下拉的固定斷言，改成依母體產生後仍成立 |
| **D-Q6** | 頁首處長提示（`#readOnlyBanner`）文案微調（見 D-7） | 已改：只改文字，未改版面 / 顏色 / 顯示條件，未觸及兩個既有區塊 | team lead：確認此微調在「不改動既有兩區塊」的約束範圍內（該提示不屬於那兩個區塊，但屬於既有頁面內容） |

### 本輪自行決定、但**不在任何 AC 之中**的內容（供 spec-writer 決定是否轉為逐字 AC）

- 條件截斷門檻 **2**、展開控制項為就地展開按鈕（AC-LIST-04 只說「由 ui-ux-designer 定案」）
- 「全部收合／全部展開」一鍵切換的存在
- 分組列即小計列（AC-LIST-08 未規定列的形態）
- 合成分組的灰字說明句：「同時指定 2 種以上產品類別的名單」／「未指定產品類別，或以文字比對方式指定的名單」
- 未登錄代碼分組的說明句：「此代碼尚未登錄於產品類別可選值清單」
- 區塊底部三句說明（含「多個條件為『且』的關係」與「加總可能不等於 100%」）
- 處長標示的**第三個觸點**（總計列後綴「（全公司口徑）」）——AC-LIST-11 只要求「明確語意標示」，未要求出現幾次
- 單一名單模式下佔比欄標題的灰字副標與底部替換說明句

## D.8 無障礙設計（WCAG 2.1 AA）

- 分組摺疊、條件展開皆為真正的 `<button type="button">`，帶 `aria-expanded`；分組按鈕另帶 `aria-controls` 指向對應的名單列容器
- 重繪後把焦點還給觸發的那顆按鈕（`applyListOverviewFocus()`），鍵盤操作不會被彈回頁首
- 分組標題格為 `<th colspan="3" scope="colgroup">`，螢幕閱讀器可辨識為群組標題而非資料格
- 「未能估算」不只用顏色表示：同時有「—」、文字徽章「未能估算」與 `title`「本次未納入合計」
- 焦點樣式一律 `focus:outline-none focus:ring-2 focus:ring-primary/30`，與頁面既有控制項一致
- 對比度見 D.3；灰字僅作輔助性重述，不作為唯一傳達管道

## D.9 驗證方式（本輪已執行）

以 Playwright（headless Chromium）＋本機靜態伺服器實測 `30-stage0-estimate.html`：

| 驗證 | 結果 |
|---|---|
| console / pageerror | **0** 筆 |
| 預設場景（3 筆母體） | 三個分組（汽車 / 機車 / 未分類），總計 28,500 —— **與既有 KPI「本月全名單總量」相同**，且既有場景數字全部未變 |
| 完整分組場景（11 筆） | 六個分組，順序 = 汽車 → 機車 → 一般商品 → `09`（未登錄代碼） → 多重產品類別 → 未分類；佔比 34/16/6/2/12/29 = **99%**（刻意） |
| 空分組隱藏 | 預設場景下「一般商品」「多重產品類別」不出現（名單數 0） |
| 組內全數未能估算 | 「機車」組名單數 1、小計 0、佔比 0%、**仍顯示**，並帶「本組合計未涵蓋 1 張未能估算的名單」 |
| 全數未能估算 | 預估數量總計 0，六組佔比全部「—」（無 0% / NaN / Infinity） |
| 空狀態 | 只渲染空狀態卡片，無分組列 / 名單列 / 小計 / 總計 |
| 處長視角 | 名單集合與部長**完全相同**（11 筆、六組、總計 47,000）；`全公司口徑` 徽章、區塊內說明條、總計後綴三處皆在 |
| 處長無轄區 | 本區塊仍完整呈現（搬至降級卡片下方），語意標示仍在 |
| 單一名單鑽探 | 只剩該名單所屬的 1 組 1 列；佔比全部「—」＋欄標題灰字說明 |
| 條件就地展開 | 4 條條件的名單由顯示 2 條 → 4 條，展開後焦點回到同一顆按鈕 |
| 分組收合 | 名單列隱藏、**小計仍可見**（13,800），焦點回到分組按鈕；「全部收合」後可見名單列 0、分組列 6 |
| 跨區塊一致性 | 本區塊 `data-total-estimated` = 30,500 ＝ 同頁 KPI「本月全名單總量」30,500（`I-F120-03`） |
| 術語黑名單 | 對本區塊 `innerText` 掃描 §9.1 ＋ F049 §19.1 全部字串：**0 命中**（含以字界正規式檢查獨立的 `IN`） |
| RWD | 1440 / 1280 兩寬度：頁面無水平捲動、表格容器亦未觸發水平捲動；名單列高一致（約 47px） |

## D.10 交付檢核

- [x] 第三個區塊完成，可直接以瀏覽器操作
- [x] OQ-F120-U1 / U2 / U3 三項皆已定案並落進原型（非僅文件敘述）
- [x] 五個分組固定順序 ＋ 未登錄代碼分組的插入位置
- [x] 佔比規則（分母、四捨五入、總計 0 → 「—」、不強制湊 100%）
- [x] 空分組依**名單數**隱藏；組內全數未能估算的分組**仍顯示**
- [x] 條件字串由 `formatConditionSummary()` 單一來源產生（自 27 逐字複製）
- [x] 處長全量可見 ＋ 三個觸點的語意標示
- [x] 正常 / 空狀態 / 部分未能估算 / 全數未能估算 / 處長 / 處長無轄區 / 單一名單鑽探皆為**可點的真實狀態**
- [x] 未新增頁面 / 路由 / 側邊欄項目；既有兩區塊的行為與版面未動
- [x] 供 constraint ring 使用的 `data-testid` / `data-*` 掛點
- [x] console 零錯誤 ＋ 兩寬度 RWD 實測
- [ ] **人工審閱閘**：D-Q1 ~ D-Q6 六項待裁決（D-Q1 / D-Q4 / D-Q6 需 team lead，D-Q2 需 spec-writer）
