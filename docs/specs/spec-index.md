...
> - **新建圖表**：[diagrams/F119-categorical-operator-flow.mmd](diagrams/F119-categorical-operator-flow.mmd)（Flowchart；operator 解讀 → data_source 分派 → NULL 語意分流 → 五路徑共用 + 簽章子圖。**未經 mermaid 工具驗證**，本機無 mermaid CLI）
> - **核心裁定（改變或補齊了 US-183 之字面契約，須人工確認）**：①**D-1 採 categorical 子屬性**（新增 optional `operator` / `keyword` 兩個 key）而**非**新增 `fieldType`——後者會迫使同一欄位在「勾選可選值」與「文字比對」間二擇一，直接違反 US-183 AC-1；`condition_payload` 為 JSON 欄位，**不需 migration、不需資料遷移**②**D-6 一致性範圍自「三處」擴為「五條執行路徑」**——逐一查證 `buildStage1WhereConditions` 之呼叫端共 5 處（MSSQL 下推 / PG 下推 / JS filter chain / Stage 0 估算 / 草稿抽樣估算），US-183 之「三處」為業務視角分類③**D-7 重複判定簽章採 `:catop:` 區段**（`欄位:catop:運算子:關鍵字`），與 `in` 形態之 `欄位:cat:values` **前綴互斥故可證明無碰撞**；無 `operator` 之舊 payload 輸出與現行**逐字元相同**，須以新舊比對回歸測試證明而非推論
> - **新增 2 條 US-183 未明載之 AC**：**AC-6**（後端互斥防呆，AC-5「不得殘留一併送出」之伺服器端保證，違反回 422 而非靜默丟棄）、**AC-11**（零可選值之 categorical 欄位仍須可加入條件——否則 US-183 之核心動機「為無法窮舉可選值之欄位提供篩選手段」無法達成）
> - **不新增錯誤碼**（全數重用 `VALIDATION_ERROR` 422 + 既有 `LIST_NO_DUPLICATE`）、**不新增端點**（受影響之 10 個既有端點見 F119 §5.2）、**無 migration**
> - **⚠ 交 system-architect（7 項，見 [F119 §12.1](features/F119-categorical-text-match-operators.md)）**：**SA-1 `data-model.md` 之 `condition_payload` schema 補述 `operator` / `keyword`（必辦，該檔非 spec-writer 之地盤，本輪未改）**；SA-2 LIKE 跳脫之技術手段（MSSQL `[` 與 PG 語意差異）；SA-3 PG builder 是否同步擴充；SA-4 `normalizeConditionPayload` 抽為共用純函式；SA-5 AC-13 修正範圍認定（＝US-183 OQ-183-03，建議不分觸發原因一律渲染）；SA-6 效能防護（US-183 OQ-183-02）；SA-7 快照序列化往返
> - **刻意未動（沿用 [F118 §12.2](features/F118-copy-from-prev-month-duplicate-indicator.md) 慣例）**：[F050](features/F050-create-list-definition.md) §5.4 規則表 / BR-6 / BR-7 共 3 處需加性補述（建議文字見 F119 §12.2），因 F050 為 P0-MVP、承載 14 個來源 Story、且其 §5.4 為多份下游 spec 之權威段落，跨 feature 條文變更應由 team lead 核可後統一為之
> - **本輪查證發現之既有落差（未回填）**：①**`spec_name`（US-183 之主要業務範例欄位「主約專案名稱」）不在部署 seed `pooldata-field-whitelist.json` 內**——使用者須先經 F075 新增流程加入白名單，本 feature 不負責 seed（F119 §3 A-1）②**F114（`customer_financial` 來源篩選欄位）無 spec 檔**（`features/` 無 `F114-*.md`，但程式碼 `stage1-customer-financial-clause.ts` 檔頭明載 F114 且已實作）——F119 對該來源之要求以程式碼不變式 `I-CF-NULL-EXCLUDE-01` 為權威依據③US-183 AC-16 末段交叉引用誤植（「AC-15」應為「AC-16」，v1.1→v1.2 順移殘留），**純筆誤不影響業務裁定**，已回報 team lead，未自行修改 US-183
> - **刻意未動（邊界）**：`data-model.md` / `architecture-spec.md` / AD（system-architect）；`prototypes/27a` `27b` `30`（ui-ux-designer）；code / test（tdd-implementation / test-generator）；`overview.md` / `nfr.md` / `scope.md`（無系統層新目標；效能要求為 feature 層）

> **🔴 v3.39 / 2026-08-13 / F116 v1.1 → v1.1.1（職稱來源更正，US-182 變更紀錄）**：使用者實機檢視後更正——職稱來源 `ob_emphire.jfun_nm` → **`ob_emphire.title_name`**，API 欄位 `jfunNm` → **`titleName`**。**F116 BR-5 整條反轉**：現為「來源唯一 `title_name`、**禁用** `jfun_nm`」（下方 v3.38 banner 之敘述已作廢，以本條為準）。理由：dev MSSQL 317 筆 `ob_emphire` 實測，`jfun_nm` 91%（289/317）為「營業一般職」屬**職能分類**、於樞紐列標籤無辨識度；`title_name` 才是真正職稱（業務專員 126／業務專員(電銷) 98／業務襄理 27／業務副理 15…共 13 種）。**兩欄 NULL 筆數相同（各 12），故職稱缺值邊界語意（AC-8 / BR-10）完全不變**。定性為**需求層更正**，非實作缺陷；未牽動新人判定／總計欄位置／工作天模式之任何裁定；不新增/移除欄位（純更名）、不需 migration、不新增錯誤碼。
> - **本輪改檔**：[F116-snapshot-pivot-analysis.md](features/F116-snapshot-pivot-analysis.md) v1.1→**v1.1.1**（新增 §1.2 變更紀錄含實測分佈表；BR-5 反轉；AC-6 / AC-8 / §5.1 SELECT 說明 / §5.1.1 契約表 / 運算歸屬表 / §7 / §10 T-4 / §11 邊界矩陣 全數更名）；`data-model.md` v1.20→**v1.21**（`ob_emphire` F116 使用模式段職稱來源反轉 + 實測佐證）。
> - **⚠ 交 system-architect（本輪新增）**：`architecture-spec.md` 之 pivot 契約段落（§ 流程圖節點寫有 `jfun_nm` / `jfunNm`）與 [`implementation-log/AD-E07-49-f116-v1.1-pivot-newcomer-workday.md`](implementation-log/AD-E07-49-f116-v1.1-pivot-newcomer-workday.md) 之 SQL 設計 / 型別 / 不變式章節需同步為 `title_name` / `titleName` —— **非 spec-writer 之檔，未改**。既有實作與測試（`assignment-run-report.service.ts`、`snapshot-pivot-view.tsx`、`implementation-log/F116-impl.md` 之 TS-F116-010 反向斷言）由 tdd-implementation / test-generator 同步。

> **v3.38 / 2026-08-13 / F116 v1.0 → v1.1（US-182，樞紐分析頁籤 UX 精修）**：依已核可 US-182（OQ-1~OQ-8 於 2026-08-13 人工閘門全數裁定）**升版既有 [F116-snapshot-pivot-analysis.md](features/F116-snapshot-pivot-analysis.md) 為 v1.1**（不開新 F 編號、不改檔名、不改端點路徑、**不需 migration、不新增錯誤碼**）。三項增修：①員編列「員編－姓名－**職稱**」＋**新人**標註（職稱來源見 v3.39 更正；新人判定基準日＝run `project_workym` **月初**、判定式 `hire_date > 基準日 − 3 個月` **嚴格未滿**、`hire_date` NULL 不標註、**不做在職過濾**——離職者仍完整顯示）②「總計」**欄**最右→**最左**（總計**列**維持最下不動）③新增「**整月／工作天**」第二維度（`workingDays` = `ob_calendar.rest_flg='0'` 落於 `project_workym` 當月之列數；每格 `ceil(整月計數 ÷ workingDays)`，**每格獨立套用**故總計不必然等於各格之和；合法組合僅 `整月-計數`／`整月-佔比`／`工作天-計數`；`workingDays = 0` → 全表 `-`，禁 `NaN`/`Infinity`）。
...
> - **API 契約（`GET /api/v1/assignment/runs/:runId/pivot`）僅新增欄位、無破壞性變更**：top-level `projectWorkym` / `workingDays`；`depts[].emplids[]` 新增 `titleName`（`string | null`，v3.39 由 `jfunNm` 更名）/ `isNewcomer`（`boolean`）。
> - **支援文件更新**：`data-model.md` v1.18→**v1.20**（`ob_emphire` 補「F116 使用模式」段：職稱來源取錯欄位警示（方向見 v3.39）、`hire_date` 新人判定、**禁止**在職過濾、重複列防禦；`ob_calendar` 補 F116 月工作日數規則與 `workingDays = 0` 降級語意；兩表相關功能補列 F116）。**無 schema 變更**。
...
> - **⚠ 交 system-architect 之 5 項（F116 §12）**：A-1 工作天換算運算歸屬（spec 預設**前端換算**，延續 BR-3）／A-2 `workingDays` 是否複用 `stage0-estimate.service.ts` 之 `resolveCalendarDay`（跨模組依賴 `assignment` → `assignment-list`，語意須與 F049 不分叉）／A-3 是否加回 `hireDate`（預設不回）／A-4 `ob_emphire` 重複 `emp_id` 裂列防禦（`title_name`/`hire_date` 進 SELECT 即須進 GROUP BY）／A-5 維度狀態是否持久化（預設否）。
...
> - **升 v2.4**：[F050-create-list-definition.md](features/F050-create-list-definition.md)（新增 AC-18 草稿「預估命中筆數」改真實抽樣估算（取代前端假公式 `12500 * 遞減係數`）+ §6.3 `POST .../preview-hit-count`（接受未儲存 condition_payload）+ BR-15；範圍限欄位篩選子步驟 `buildStage1WhereConditions`（D2，不含 MONTH_CNT / 去重 / 特殊 DELETE，與 F049 精確試算互補）；僅建立頁，`list-edit-draft` 為 deferred follow-up；US-176）
> - **刻意未動（邊界，交 system-architect / 其他 agent）**：**AD-E07-45 抽樣估算**（抽樣演算法 / 固定樣本大小 / 可重現種子產生 / 樣本→母體放大推算公式 / MSSQL SQL 下推 / 是否短期快取 — system-architect 撰寫）；`architecture-spec.md` §18.5（`buildStage1WhereConditions` 既有）；prototype 28 Tab 5 分布面板 mock + 文案（ui-ux-designer）；code / test（tdd-implementation / test-designer 範疇）
> - **本輪殘留 open question**：(1) F056 §5.5 唯讀預估於分派執行中可讀（US-175 AC-6）與 F055 §5.2 現仍回 409 之不一致待對齊（F056 A-8，交 system-architect / PO）；(2) `list-edit-draft` 是否比照補預估命中筆數面板（F050 §9.2 / US-176 開放問題，另立 Story）；(3) prototype 27a「基於上月案件樣本估算」文案是否據實改為「當前 Pool 資料」（ui-ux-designer）
>
> **v3.33 / 2026-07-09 / F110 新增泛用 ETL 節點 `code_decode`（單趟多欄位代碼解碼，US-173）**：依已核可 US-173 **新建 F110**（E05 ETL Pipeline）。本輪變更檔案：
> - **新建 v1.0**：[F110-etl-code-decode-node.md](features/F110-etl-code-decode-node.md)（泛用轉換節點 `code_decode`，為 `lookup`（F043 §4.8）之**泛用化**：對**同一張**對照字典表在**一次資料流掃描**中以任意數量、任意 filter 的多組「代碼 → 描述」mapping 一次解碼，取代一組打同一字典表、各自對大分支（≈360 萬列）做就地全表 UPDATE 的 `lookup` 節點。**config schema**：節點級單一共用字典來源（`lookupRef`/`lookupSource`，解析同 lookup）+ `mappings[]`（per-mapping `matchColumn`/`lookupMatchColumn`/選用 `filter`/`outputColumns` 清單，欄位名刻意沿用 lookup）；**固定 LEFT JOIN/NULL 語意**（不提供 noMatchStrategy/defaultValue）；TRIM/cast/重複 key 取首筆逐一對齊 lookup。**逐格等價契約（US-173 AC-2 硬性）** + 「N 個同字典 lookup ⇔ 1 個 code_decode」雙向決定性對應（供 EQ 測試與 migration `down()`）。**spec 層兩問題裁定**：(a) 單一 mapping 表允許用 code_decode 且 customer_core 一律採用、(b) filter = 同 `lookupFilter` 之自由格式 SQL 布林式（單一等式/複合/無）。informative 附錄：customer_core 5 個字典表實例 → 9 個 code_decode 節點 / 31 mapping（涵蓋全部 filter 型態與 mapping 組數）。`lookup` 節點 additive 不變。）
> - **既有 spec 更新（本 feature 負責）**：[F043-etl-node-executors.md](features/F043-etl-node-executors.md) v1.3→**v1.4**（§1 節點數 8→9 + 加 `code_decode`、`target_load` 改列第 10 種；**新增 §4.9 CodeDecodeExecutor** 目錄摘要 mirror §4.8 格式、cross-ref F110；§6 補 4 個 code_decode 邊界；§8 相關文件加 F110）
> - **無新錯誤碼**：`code_decode` 沿用 lookup 既有節點級失敗慣例（節點 `'failed'` + errorMessage），審查結論不新增 error-handling.md 錯誤碼（F110 §13）
> - **無新 Mermaid 圖表**：語意以 F110 §6 / §7 表格 + 決定性對應表承載（比照 F103 慣例，Mermaid 數不 +1）
> - **刻意未動（邊界，交 system-architect / 其他 agent）**：`architecture-spec.md` / AD-E07-41（`code_decode` 實體執行策略：`SELECT INTO` vs 就地 `UPDATE` / `##temp` 生命週期 / MSSQL·PG 方言 SQL / temp table 架構 = F110 §15 OQ-F110-01~05）；`data-model.md`（無新 entity；`code_decode` 為 pipeline 定義 JSONB 節點，非資料表）；code / handler（`code-decode-handler.ts` + `-mssql.ts`）/ NodeExecutor 註冊 / pipeline 定義 migration（含 `down()` 還原舊 lookup 鏈）/ test（tdd-implementation / test-designer 範疇）
> - **本輪對使用者無殘留 open question**（US-173 業務範圍待解決問題已由利害關係人確認；「尚待 Spec 層決定」兩項已於 F110 §6.4 裁定落地；F110 §15 OQ-F110-01~05 均屬架構師 HOW，附建議預設）
>
> **v3.32 / 2026-07-08 / MSSQL 全面遷移 P6（生產切換 cutover）計畫**：P5 全部完成（P5h/P5g 修法+重驗、P5e 簽核報告已產出、P5i 裁定完成），使用者裁定進入 P6。新增 **[`implementation-log/AD-E07-44-mssql-p6-cutover.md`](implementation-log/AD-E07-44-mssql-p6-cutover.md)**。**定位修正**：project memory「無 prod 資料」——PG 從未是正式生產庫，P6 實質是「以 MSSQL 為地基之首次正式上線」（從 legacy 來源重新 ETL，非搬遷既有資料），大幅降低傳統熱切換之資料風險。**🔴🔴 本輪規劃新發現、最高優先序之 P6-0 硬性前置閘**：交叉核對 `AD-E07-38` 明載之「目標=SQL Server 2022，不做 2016 相容改寫」前提，與 project memory 記載之後續發現「實機可能是 **2016 SP3 Standard**／2016 缺 `TRIM()`（2017+ 語法）需改 LTRIM+RTRIM ~20 站點」——此落差自發現後**未見任何後續 AD 或 impl-log 回頭處理**。**本次直接 Grep 查證確認 `TRIM(` 已廣泛用於 6+ 個 MSSQL SQL 產生層核心檔案**（Stage 1 篩選/Stage 2 計分/ETL lookup·type_cast·target_load handler/暫存表共用工具，含一處共用 helper 非單一站點），若真實生產版本確為 2016，**P1-P5 全部驗證基準（2022 容器）與實機不符，需 TRIM 全站點改寫 + 針對真實 2016 環境完整重驗**。裁定此為 architect 無法自行確認之基礎設施事實，**列為 P6 全流程硬性前置閘**（P6-0），需使用者提供真實版本之權威確認。**移除範圍界定**：區分 (a) 部署切換（docker-compose/env 指向 mssql，滿足「no hybrid」生產不雙引擎並存之硬約束語意）與 (b) 程式碼消除（移除 `pg`/`pg-boss`/`pg-copy-streams` 內部使用 + PG 版 handler/builder），**建議分階段、(a) 先行、(b) 待 P6e 上線後保留觀察期（≥1-2 個月名單分派週期）再執行**——理由：PG 版程式碼於 `DB_TYPE≠postgres` 時已證實「建立但不使用、無害」（P2/P5h 既有證據），保留具除錯安全網價值，且刪除屬不可逆操作應延後聚合執行；若使用者傾向一次做完，底線為「P6f 不得早於 P6d 業務簽核通過」。**重要範圍修正**：`postgresql-executor.ts`／`connection-tester.provider.ts` 之 postgresql 分支**不屬於**本次遷移範圍——查證確認其為 `ExecutorFactory` 服務之**外部 Datasource 來源型別支援**（使用者可設定任意外部 PG/MySQL/MSSQL 來源系統做 ETL 擷取），與 CDMP 自身內部資料庫引擎無關，`pg` npm 依賴因此**不可移除**（`pg-boss`/`pg-copy-streams` 可移除）；此為與協調者原始問題框架不同之修正，另立獨立產品範圍決策事項供裁示。**子切片順序**：P6-0→P6a（MSSQL bootstrap，即原 P5f 併入）→P6b（docker-compose 切換）→P6c（生產首次 ETL 灌入）→P6d（正式月名單分派+F067 式對真實 legacy 比對業務驗收）→P6e（正式上線）→P6f（程式碼消除）；**Point-of-no-return 明確定義為 P6e**（業務開始依 MSSQL 版結果實際派案決策之時刻，而非傳統「舊系統停止寫入」定義，因無既有生產資料可供比較）。新增 4 個不變式（`I-MSSQL-VERSION-CONFIRMED-01`/`I-MSSQL-NOHYBRID-PROD-01`/`I-MSSQL-SOURCE-EXECUTOR-SCOPE-01`/`I-MSSQL-CUTOVER-REVERSIBLE-01`）。估算：8-13 人天（不含 P6-0 修復）／13-23 人天（若確認 2016 需修復）。4 項需使用者裁示事項（版本確認、分階段裁量、上線時機、postgresql-executor 產品範圍）。**刻意未動**：`architecture-spec.md`（理由同前，P 系列尚未 cutover 落地——**本文件即是該 cutover 之規劃，落地後應觸發 `architecture-spec.md` 之正式更新**，列為 P6f 之後續行動）。

> **v3.31 / 2026-07-08 / MSSQL P5e F067 正式簽核報告產出、P5h/P5g 修法完成、P5i varchar→nvarchar 裁定（AD-E07-43 v1.2→v1.3）**：**P5h（useUTC 連線層修法）與 P5g（ATOMIC 交易包裝）皆已實作完成並真庫重驗通過**——assignday 於 198+9,376 案樣本由全 −1 日轉為 **0-diff**、全量 mssql 回歸 673 通過零邏輯回歸；ATOMIC 修法涵蓋 fullMode/partition_replace/customer_core UPSERT 三路徑（P5g 實作中主動發現原不變式未涵蓋 UPSERT 兩段式路徑、範圍隨之擴大）、真庫探測證實 MSSQL `TRUNCATE` 於顯式交易內可回滾、P5b 原「資料遺失」斷言已翻轉為「資料保留」重驗通過；殘餘 follow-up（非阻擋）＝MSSQL 標準 Read Committed 下並行查詢載入期間會被阻塞（非讀空表，真庫實測 ~712ms，7.8M 生產規模未實測，可選 `READ_COMMITTED_SNAPSHOT`）。**正式產出 F067 式簽核報告** `implementation-log/AD-E07-43-P5e-f067-signoff.md`：10/10 關鍵欄位達成逐列一致（score 5 案為既知今日參考日效應）、誠實揭露 27,796 案大 CR 名單之 assignday 未於本輪重跑（其餘 9 欄含 CR 全鏈已 100% 驗證）、列示 4 項待知悉非阻擋事項，待業務正式簽署。**P5i（varchar byte→nvarchar）裁定完成**：架構師比對真實 legacy MSSQL 來源 schema（`reference/TableSchema/OB/OBPOOLDATA.sql`／`OBPOOLDATA_LIST.sql`／`OBEMPHIRE.sql`）逐行核對，**排除「PG→MSSQL 測試複製 harness 假象」假說，確認為真實生產風險**：legacy 明確宣告 `SPEC_NAME`/`CAR_NAME`/`BROKER`/`EMP_NM`/`DEPT_NAME`/`TITLE_NAME`/`JFUN_NM` 等欄位為 `nvarchar`（Unicode 安全），CDMP 之 schema 產生器 `apps/api/scripts/parse-ob-schema.mjs::mapType()`（第 77-87 行，原為純 PG 目標設計）將來源 `nvarchar`/`varchar(N≤255)` 一律收斂為泛用 TypeORM `'varchar'`，MSSQL 目標下遺失 Unicode 安全性，於真實中文長字串下有截斷風險（已直接驗證 `ob_pool_data`/`ob_pool_data_list` ~20+ 欄、`ob_emphire` 4 欄受影響；對照組 `DLR_NAME`/`BRNH_NAME` legacy 確為 `varchar`，佐證非隨意設計、CDMP 產生器抹平了逐欄區分）；確認不影響計分/CR/分派核心 10 欄比對結果，非 cutover-blocker。使用者已裁示「varchar 自主做」，不需另行業務裁示；架構師產出機械式修復方法論（新增 dialect-aware `nvarcharColumnType` helper + 修正產生器 + 重跑掃描取得完整清單，取代人工逐欄列舉）交 tdd-implementation 執行。**就地修訂 `implementation-log/AD-E07-43-mssql-p5-ci-signoff.md`（v1.2→v1.3）**：新增 §7.6/§7.7（P5g 完成與範圍擴張/殘餘風險）、§8.9（P5h 完成與驗證結果）、**新增 §9**（P5i 完整裁定：根因/legacy 逐行核對/影響面/修復方法論）；新增不變式 `I-MSSQL-NVARCHAR-DISPLAY-01`；擴大 `I-ETL-ATOMIC-LOAD-01` 涵蓋範圍至 UPSERT；`I-MSSQL-DATE-TZ-01` 標記已驗證滿足；§1 mermaid 圖/子切片表、§4 裁示事項表、§5 全部 DoD 同步更新完成狀態。**新增** `implementation-log/AD-E07-43-P5e-f067-signoff.md`（正式簽核報告，待業務簽署）。**刻意未動**：`architecture-spec.md`（理由同前）。

> **v3.30 / 2026-07-08 / MSSQL P5c assignday −1 日 cutover-blocker 與 AD-E07-43 v1.1→v1.2 修訂**：P5c（MONTHRUN-DIFF 真實 PG vs MSSQL 逐列比對，3 樣本 198／9,376／27,796 案）確認其餘 9 個關鍵欄位（score/tier/card/dept_id/emplid/CR 三欄等）逐列 **0-diff**（含 CR 全鏈 1,996 案），唯 `assignday` **全部系統性早一日**。架構師採**原始碼逐行追蹤**（非僅經驗推論）定位根因：TypeORM `SqlServerDriver.createPool()`（`node_modules/typeorm/driver/sqlserver/SqlServerDriver.js:918-924`）在應用程式未顯式設定 `options.useUTC` 時**強制覆寫為 `false`**，蓋過 tedious 函式庫本身 `true` 的內建預設值（`connection.js:485`），導致 MSSQL `date`/`datetime`/`datetime2`/`smalldatetime`/`time` 讀寫全面改用本地時區分量而非 UTC 分量（`value-parser.js::readDate`/`readDateTime2` 等逐行確認）；本專案 4 個建構 MSSQL TypeORM 連線之站點（`app.module.ts`/`worker-app.module.ts`/`database/data-source.ts`/`database/seeds/seed-connection.ts`）**全數未設定 `useUTC`**，全部落入此缺口。**架構師推薦修法＝連線層補 `useUTC:true`**（優於逐檔案改讀取端正規化邏輯）：(1) 完全不需修改凍結檔（`stage0-estimate.service.ts`/`assignment-run-pipeline.service.ts`）；(2) 一次性讓本專案既有主流慣例（Grep 確認至少 4 個「讀 DB Date 欄位→`getUTC*()`正規化」同構站點，含 `emphire-active.util.ts`〔全系統在職判定 single source of truth〕、`cr-priority.ts`、`assignment-run-pipeline.service.ts::toYmd`〔CR 兩年門檻〕等 **2 個本次新發現、原 P5c 報告未點名**之潛伏風險站點）在 MSSQL 上成立，而非逐點打補丁；(3) PG 零回歸（`useUTC` 為 MSSQL 專屬選項）；(4) 確認與 P4e（bulk-load 8 小時偏移）**同源但範圍不重疊**——P4e 之修法繞過 TypeORM 自建連線池，未觸及本次發現之 TypeORM 主連線缺口。**統一裁定**：本次修法**一併吸收 P5d**（datetime2 時區 production 查證）之待辦——原待業務查驗證/裁決時區組態，現已根因級定位，P5d 範圍收斂為「確認 P5h 修法已套用+重驗達 0-diff」。**範圍歸屬＝新增子切片 P5h**（🔴 cutover-blocker、優先度最高，依賴 P5c 已完成，P5e 簽核依賴 P5h 完成）；附帶次要發現（varchar byte 語意，BIN collation 下中文顯示欄位元組計長溢位風險，非計分/CR/分派輸入、非阻擋）另立 **P5i**。**架構師判斷需使用者知情裁示**（cutover 前必修阻塞項，影響 MSSQL 部署下全系統日期正確性）。**就地修訂 `implementation-log/AD-E07-43-mssql-p5-ci-signoff.md`（v1.1→v1.2）**：新增 §8 完整記錄現象/根因鏈程式碼證據/修法評估表/推薦理由/影響站點盤點/與 P3d-P4e 同源判定/範圍歸屬/varchar byte 次要評估/可帶回使用者精簡摘要；新增不變式 `I-MSSQL-DATE-TZ-01`；§1 mermaid 圖與子切片表新增 P5h/P5i；§4 新增第 4/5 項裁示事項、更新第 1 項（P5d 收斂）；§5 新增 P5h/P5i DoD；§6 不變式表新增列。**刻意未動**：`architecture-spec.md`（理由同前）。

> **v3.29 / 2026-07-08 / MSSQL P5b ATOMIC 資料完整性風險（潛在封鎖級）與 AD-E07-43 v1.0→v1.1 修訂**：P5b（其餘 5 條生產 ETL pipeline 端對端 MSSQL 驗證）真庫實測發現 **ATOMIC 資料完整性風險**——`target-load-handler`（**PG／MSSQL 兩版結構對稱，非本次遷移引入**）之 fullMode（`TRUNCATE`+`INSERT`）／partition_replace（`DELETE`+`INSERT`）路徑，`pipeline-runner.ts` 全程無交易保護（TypeORM `QueryRunner` autocommit），TRUNCATE/DELETE 先提交、後續單句 INSERT 若因髒資料（NOT NULL 違反／數值溢位／撞 PK）整句失敗**不會回滾**，導致目標表/分區既存生產資料被清空且未重建（真庫 4 種目標表形狀一致驗證：`ATOMIC-001~004`）；5 條 pipeline 皆無 `type_cast` 節點防線。生產影響：月度 ETL 遇單筆髒資料，`ob_pool_data`（7.8M 列）等來源表可能被整批清空，下游月名單分派讀空表。架構師評估三候選修法：**推薦 (a) 交易包裝**（TRUNCATE/DELETE+INSERT 同交易，失敗完整回滾；成本低、雙引擎對稱可落地、與既有單句大陳述式架構相容、且 Read Committed 隔離下已能取得 (b) 方案「中間態不可見」的主要效益）為必要立即修法；**(c) 前置驗證**（TRY_CAST/NOT-NULL 壞列導向 skip）為緊接之獨立 hardening（(a) 單獨無法解決「來源恆有髒列則整表載入從此卡住」的可用性問題）；**(b) Swap 全量**記錄為已評估但不建議之選項（額外儲存/複雜度成本高、主要效益已由 (a) 取得）。**範圍歸屬裁定**：獨立新增子切片 **P5g**（非阻擋既有 P5a~f/P5c MONTHRUN-DIFF/P5e F067 簽核之推進），因兩引擎共通（非 MSSQL 遷移回歸）但藉本次嚴謹端對端測試方法論才被驗證揭露，且在既有 PG/MSSQL 平行維護框架下修法成本低，不建議與遷移脫鉤處理。**架構師判斷需使用者知情裁示**（生產行為改變提案：失敗語意由「靜默清空」改為「安全失敗、舊資料保留」，且現行 PG 生產系統今天就受影響，是否/何時修屬業務風險接受度判斷）。**就地修訂 `implementation-log/AD-E07-43-mssql-p5-ci-signoff.md`（v1.0→v1.1）**：新增 §7 完整記錄發現/修法評估表/推薦理由/範圍歸屬/可帶回使用者之精簡摘要；新增不變式 `I-ETL-ATOMIC-LOAD-01`；§4 需使用者裁示事項新增第 3 項；mermaid 圖新增 P5g 節點。**刻意未動**：`architecture-spec.md`（理由同前）。

> **v3.28 / 2026-07-08 / MSSQL 全面遷移 P5（全量 CI + F067 式業務簽核）計畫**：P3（Raw SQL 引擎移植，3a~3e）全數完成並 push——**MSSQL 月名單分派全鏈（Stage1 篩選→Stage2~3 計分→Stage3/4 比例→CR）現已全部有值**。進入 cutover 前最後關卡 P5：非新業務邏輯，而是「證明 MSSQL 忠實重現 PG」之全量驗證+業務簽核。新增 **[`implementation-log/AD-E07-43-mssql-p5-ci-signoff.md`](implementation-log/AD-E07-43-mssql-p5-ci-signoff.md)**，固化：(1) **🔴 P5a（優先度最高、槓桿最大）CI mssql-specs 缺 dbo baseline bootstrap 之修法**——查證確認 `.github/workflows/ci.yml` 的 `mssql-specs` job 僅建空 `CDMP_TEST` 資料庫、未跑 `migration:run`，導致 P3a 的 52 個案例被 GATE-002 靜默 skip（CI 綠燈但實際未真跑）；裁定**加 CI 層 `migration:run` bootstrap 步驟（非讓 P3a 回頭改自建）**，理由：一次修復全面受益+驗證 migration 檔案本身正確性+與 P3b~d 既有雙模式 harness 相容成本最低；另需一併排查已知的 P1b2「wipe dbo」測試排序風險；(2) P5b 其餘 5 條生產 ETL pipeline（`E07-OBPOOLDATA-Load` 等，P4d 僅端對端驗證 customer_core 一條）之 MSSQL 驗證，為 P5c 前置依賴；(3) P5c **MONTHRUN-DIFF**——真實觸發 PG/MSSQL 完整月名單分派，逐列比對 score/tier/card/dept_id/emplid/assignday/cr 九欄，**manual/script 執行（比照 F101/F102/F104 前例，非新 CI 測試）**；(4) P5d datetime2 時區 production 查證（P3d 已發現 tedious 以本機時區存 datetime2，非午夜 `appl_date` 可能使逾2年清空邊界與 JS oracle 分歧）；(5) P5e F067 式業務簽核報告（基準修正為「MSSQL vs PG」而非「vs legacy」，legacy 對齊已在既有 PG 版 F067 完成）；(6) P5f（可選）MSSQL 部署 bootstrap 對齊 PG 版一鍵部署。新增 2 個不變式（I-MSSQL-CI-BOOTSTRAP-01／I-MSSQL-SIGNOFF-GATE-01，後者明訂 cutover 需 MONTHRUN-DIFF 一致 + 業務簽核雙條件）；判定不需要 spec-writer。**明確彙整 2 項需使用者/業務裁示事項**（datetime2 時區查證與裁示；F067 簽核本身），其餘子切片（P5a/b/f）判斷純技術執行可直接推進。**刻意未動**：`architecture-spec.md`（理由同前）。
...
> - **新建 v1.0**：[F109-customer-source-filter-fields.md](features/F109-customer-source-filter-fields.md)（白名單引入 `data_source` 概念〔`ob_pool_data` / `customer_core`〕+ API 暴露 `dataSource` + M06 列表來源欄 + 名單定義來源分組；新增 8 個 `customer_core` 篩選欄位〔性別 code→label / 年齡衍生 AGE 基準＝`project_workym` 月首日 / 居住城市 `LEFT(cpost_city,3)` 縣市級 / 5 個 `_desc` value=label〕；F076 seed 7 categorical 欄位可選值〔3/55/8/5/4/9/22〕；月名單分派 Stage 1 **條件式** LEFT JOIN customer_core〔`custo_no=source_customer_no`〕+ **NULL=排除** 核心語意〔BR-2/BR-3〕+ 三處消費一致〔月名單分派 / Stage 0 試算 / 名單試算，BR-10〕。US-172 4 個 OQ〔年齡基準 / 城市 seed / 性別機制 / 空表限制〕已裁示落規格）
...
> - **前置依賴**：ADD_UN_CAPITAL 維度需 `ob_arreturndf_min_cap` ETL 就緒（AD line 4093，月名單分派前置必要檢核）；customer_core / arreturndf ETL 已重做、對 pool ~100% 覆蓋、TEST_* 污染欄已清。
...
>
> **v3.13 / 2026-06-17 / F064 v2.1 匯出 pool 源血緣 bug 修正（live 抓到掉 11.5% 列）**：[F064](features/F064-export-assignment-result.md) v2.0 之 pool 欄 join 源 `ob_pool_data_list`（per-list 去重表）**錯誤** → INNER JOIN 掉 11.5% 列。根因：月名單分派 Stage 1 `INSERT INTO ob_monthly_run_result SELECT … FROM ob_pool_data o`（共享池，PK orgno+appl_no，無 list_no），`ob_pool_data_list` 僅於 Stage 1 被 LEFT JOIN 取 CR 三欄、非 result 列母體。本輪變更檔案：
...
>
> **v3.11 / 2026-06-12 / F102 月名單分派 CR 優先分派（補 F101 simplified is_cr 缺口 + per-list `cr_enabled` 閘控 + 廢除全域旗標）**：接續 [F101](features/F101-stage3-4-proportional-assignment.md)（BR-F101-12 將 is_cr 簡化為被動標記、未實作 CR 優先分配），依 3 個已核可 user story 新建 1 個 feature spec，於 F101 Stage 3/4 比例分派**之前**插入 CR 優先分派前置處理。本輪變更檔案：
...
>
> **v3.10 / 2026-06-04 / F101 月名單分派 Stage 3/4 真實比例分派（Bug C：OB202606001 全員 `emplid=NULL`）**：接續 AD-E07-28 系列（F098/F099/F100），依 5 個已核可 user story 新建 1 個 feature spec，**取代 F100 之 placeholder Stage 4**（僅取 `ob_dept_pct` 第一列電銷課 + 單一 `defaultEmpl`，全案件指向同一員工 → 該課無員工時全員 `emplid=NULL`）。本輪變更檔案：
...
>
> **v3.9 / 2026-06-02 / AD-E07-28 月名單分派執行模型重構（Worker 抽離 + Stage 1~4 SQL 下推）**：依 [architecture-spec.md §5.13 + AD-E07-28 v3.1](architecture-spec.md)（system-architect 維護，status: proposed）新建 3 個 feature spec，將月名單分派執行模型由「cdmp-api 同程序 `setImmediate` 背景跑」重構為「pg-boss 入列 → 獨立 cdmp-worker 容器消費 → Stage 1~4 set-based SQL `INSERT…SELECT`」，分 P1/P2/P3 三階段交付。本輪變更檔案：
...
> - **新建 v1.0**：[F094-monthly-run-result-table.md](features/F094-monthly-run-result-table.md)（單源化 Phase A — 月名單分派結果表 `ob_monthly_run_result` + pipeline 落點切換，migration `1711360000292`）、[F095-applied-special-rules-readonly.md](features/F095-applied-special-rules-readonly.md)（特例規則前端唯讀 `appliedSpecialRules[]` 讀時推導，無新 DB 欄位）、[F096-pooldata-whitelist-list-type-cleanup.md](features/F096-pooldata-whitelist-list-type-cleanup.md)（白名單 `list_type` 停用，migration/seed `1711360000293`）
> - **升 v2.0**：[F091](features/F091-stage1-complete-month-cnt-dedup-special-delete.md)（**high-severity bug fix**：特例 DELETE trigger 由 mojibake 誤判「中結強案/中結/年資+滿」修正為 SP 正確版「**期中機車/期中/年以上+小資/白牌**」；去重上界 `MIN(MAX(assignday), workdt−1日)`；year_produ 補 `parseInt`；月名單分派寫入目標改 `ob_monthly_run_result`）、[F090](features/F090-obpooldata-list-etl.md)（單源化：`data_source` 值域 `'etl_load'`、月名單分派不再寫本表）
> - **升 v1.1**（note-only 同步）：[F092](features/F092-stage1-dry-run-estimate.md)（dry-run 同步 F091 v2.0 修正後 trigger / 去重上界 + 月名單分派落點 `ob_monthly_run_result`；唯讀行為不變）
...
> - **⚠️ Production 影響（Phase A 群組）**：F091 v2.0（特例修正）+ F094（落點切換）+ F095（前端唯讀）**同批 deploy、無 feature flag、deploy 後直接生效**（DP-AD23-2 / DP-AD26-1）。F091 v2.0 之 trigger bug fix 將顯著改變各類名單案件數（含「中結/強案/年資」名單案件數增加、含「期中機車/期中小資/年以上」名單案件數減少），**須 deploy 前業務知會 + 各類名單案件數差異驗收**。F090（ETL）/ F092（dry-run）/ F096（白名單清理）不改月名單分派案件數。
...
>
> **v3.5.1 / 2026-05-26 / F092 落地後 estimate 語意漂移同步（F049 → v1.4、F088 → v1.3.1）+ F090 歷史限定欄位修正**：F092 已落地（estimate 改為完整 `Stage1FilterChain` dry-run，≡ 月名單分派），本輪同步既有 spec 之 estimate 語意文字漂移（只動 docs，不碰 code/test）：
> - **F049 升 v1.4**：[F049-stage0-daily-estimate.md](features/F049-stage0-daily-estimate.md) BR-6 由「估算為條件符合上界」改為「**完整 Stage 1 預估（≡ 月名單分派案件數）**」（已含 month_cnt + 去重 + 特殊 DELETE）；AC-4 / §5.2 對齊（複用 `executeStage1Chain({dryRun:true})`）；保留 BR-1（最終以月名單分派為準）；交叉引用 F091 / F092 / AD-E07-23。
...
> - **F049-test 升 v1.4**：[F049-test.md](../test-specs/features/F049-test.md) TS-F049-EST-010 預期值 `≈ 241,978`（欄位篩選版）過時 → 改為「dry-run COUNT === 月名單分派（不 assert 固定值）；完整鏈後 ≤ 241,978」，標 Integration DEFERRED（需真實 PG + ob_pool_data_list seed）。
...
> - **⚠️ Production 影響**：F091（Phase 2）為唯一改變 production 月名單分派案件數之階段，**無 feature flag、deploy 後直接生效**（DP-AD23-2），須 deploy 前業務知會。F090 / F092 不影響月名單分派案件數。
...
| [nfr.md](nfr.md) | 非功能需求（安全性、效能、E07 月名單分派執行效能/快照原子性/結果準確性） | Architect, TDD, QA |
...
| F043 | [F043-etl-node-executors.md](features/F043-etl-node-executors.md) | ETL 節點執行器（9 種節點處理邏輯與 TypeScript interfaces，含 Lookup 雙輸入模式、§4.9 CodeDecodeExecutor 摘要） | US-056, US-057, US-058, US-173 | P0-MVP |
| F044 | [F044-etl-target-load.md](features/F044-etl-target-load.md) | Target Load + UPSERT（批次寫入、ETL 追蹤欄位） | US-057 | P0-MVP |
| F110 | [F110-etl-code-decode-node.md](features/F110-etl-code-decode-node.md) | Code Decode 節點（泛用單趟多欄位代碼解碼；lookup 泛用化，同一字典表 N 組 mapping 一次 LEFT JOIN；逐格等價 lookup 鏈） | US-173 | P0-MVP |
...
| F050 | [F050-create-list-definition.md](features/F050-create-list-definition.md) | **草稿階段建立名單定義（v2.4 / 2026-07-11 / US-176：草稿「預估命中筆數」改真實抽樣估算 — AC-18 + §6.3 `POST .../preview-hit-count`（接受未儲存 condition_payload）+ BR-15，取代前端假公式 `12500 * 遞減係數`；範圍＝欄位篩選子步驟 `buildStage1WhereConditions` ＋ F109 customer_core 條件式 LEFT JOIN 子句（team-lead 2026-07-11 override，客戶欄位須納入，否則高估）；仍不含 MONTH_CNT / 去重 / 特殊 DELETE（D2，與 F049 精確試算互補）；僅建立頁，`list-edit-draft` deferred；v2.3.1 US-144 最低條件數修正 — 「≥1 條件」門檻僅計非系統固定 condition，best_case 不計入；v2.3 best_case 系統固定條件 BR-14 `injectSystemFixedConditions` + AC-17；v2.1 whitelist-driven 重構：`condition_payload` 為 source of truth + `CONDITION_COLUMN_NOT_IN_WHITELIST` + `RESERVED_FIELD_IN_CONDITIONS` + `LEGACY_LIST_NOT_COPYABLE`）** | US-106, US-107, US-120, US-121, US-125, US-144, US-176 | P0-MVP（**v2.4**）|
...
| F055 | [F055-edit-card-level-thresholds.md](features/F055-edit-card-level-thresholds.md) | 編輯 CARD_LEVEL 分級門檻（M02 Tab 4）**（v1.8 / 2026-07-12 / US-174：§5.2 預覽改分數直方圖 + 前端分桶（AD-E07-45 v1.2）— response 加 `histogram: [{score,count}]`（與門檻無關）、`levels` 改選填、前端每 cardType 取一次快取後依門檻即時前端分桶（門檻編輯不再打後端）；效能＝直方圖掃描 heavy card 約 12 秒（取代 224.6 秒）/每 cardType 一次快取、前端重分桶即時；BR-2/BR-8 改寫。v1.7：AC-3 預覽改抽樣估算 + AC-8 估算失敗三態錯誤重試（修 `catch { setPreview(null) }` 靜默吞噬空白）+ §5.2 補 `isEstimate`/`sampleSize`/`totalCount`）** | US-074、US-097、US-174 | P0-MVP | v1.8 |
| F056 | [F056-edit-tier-mapping.md](features/F056-edit-tier-mapping.md) | 編輯 TIER_LEVEL 對應表（M02 Tab 5）**（v1.7 / 2026-07-12 / US-175：各 TIER 分布互動路徑改前端衍生自 F055 §5.2 快取分數直方圖（依已儲存 CARD_LEVEL 門檻分桶→map ob_tier→彙總→放大），切至 Tab 5 不觸發新掃描/不呼叫 §5.5、重新彙總即時（次秒級）；§5.5 端點重定位為 canonical server API（非互動路徑）；AD-E07-45 v1.2。v1.6：新增 AC-10~13「預估各 TIER 分布」唯讀面板 + §5.5 `GET .../tier-mapping/preview`（`hasMapping`/`ruleType`/`distribution[]`、不回 409、執行中可讀）+ BR-14；全新 UI（D3），prototype 28 Tab 5 mock 交 ui-ux-designer）** | US-075、US-175 | P0-MVP | v1.7 |
...
>
> **F054 ⇄ F106 對稱關係**：F054 提供「停用維度」（disable，`status active→inactive`），F106 對稱補完「顯示停用維度 + 重新啟用」（enable，`status inactive→active`）；兩端點除狀態方向 / 動詞 / 時間戳欄名外，guard / feature flag / 月名單分派鎖 / audit / 404 語意完全一致（見 F106 §5.3 對稱性對照表）。
>
...
| Feature ID | 文件 | 標題 | 來源 Story | 優先級 |
|------------|------|------|-----------|--------|
| F088 | [F088-ready-stage-summary.md](features/F088-ready-stage-summary.md) | **準備完成階段查詢摘要（部長 + 處長 + Admin 唯讀；含篩選 / 部門比例 / 個別業務比例 / CR 開關四區塊；處長轄區過濾；月名單分派前置條件即時計算；v1.1 補 `proxyStatus` 欄位 schema 對應 F084-A 落地）** | US-118 | P0-MVP（**v1.1**）|
...
| Feature ID | 文件 | 標題 | 來源 Story | 優先級 |
|------------|------|------|-----------|--------|
| F061 | [F061-trigger-assignment-run.md](features/F061-trigger-assignment-run.md) | **觸發分派月名單分派（Stage 0~4 + 三份快照原子性寫入；v1.2 PO 決議 OQ-E07-29-A 邊緣 CARD_TYPE HB/SEB/SEC 跳過 + report_payload.skippedCases JSONB + 月名單分派仍 completed；v1.1 補「所有 active 名單 stage = 'ready'」前置條件 + Stage 3 CR 路徑改 per-LIST_NO `cr_enabled` + `MONTHLY_RUN_BLOCKED_LIST_NOT_READY` 錯誤碼）** | US-081 | P0-MVP（**v1.2**）|
...
> **v3.6 / 2026-05-27 / AD-E07-25 + AD-E07-26（architecture-spec v2.18）**：在原 Stage 1 精確化三階段（F090~F092）之上，依 AD-E07-25（資料單源化）+ AD-E07-26（特例規則 SP 落差修正）新增 F094~F096 並升版 F090/F091/F092。**⚠️ Phase A 群組（F091 v2.0 + F094 + F095）同批 deploy，無 feature flag、deploy 後直接生效（DP-AD23-2 / DP-AD26-1），改變 production 月名單分派各類名單案件數，須 deploy 前業務知會 + 案件數差異驗收。** 詳見各 feature §13。

| Feature ID | 文件 | 標題 | 來源 | 優先級 |
|------------|------|------|-----------|--------|
| F090 | [F090-obpooldata-list-etl.md](features/F090-obpooldata-list-etl.md) | **Phase 1 + Phase B 單源化 — OBPOOLDATA_LIST ETL 載入 + `data_source` 單源化**（雙層 ETL；**v2.0**：`data_source` 值域單一化 `'etl_load'`、月名單分派改寫 `ob_monthly_run_result` 不再寫本表、Load 前置 DELETE 可全量；歷史限定 `ASSIGNDAY < 本月第一天`；ETL 不影響 production 月名單分派案件數） | AD-E07-25 / US-133 | P0-MVP（**v2.0**）|
| F091 | [F091-stage1-complete-month-cnt-dedup-special-delete.md](features/F091-stage1-complete-month-cnt-dedup-special-delete.md) | **Phase 2 + Phase A 特例修正 — Stage 1 補完整 + 特例 DELETE SP 修正**（MONTH_CNT 期別 + 近 3 月去重 + 特例 DELETE；**v2.0 high-severity bug fix**：trigger 由誤判「中結強案/中結/年資+滿」修正為 SP 正確版「**期中機車/期中/年以上+小資/白牌**」、去重上界改 `MIN(MAX(assignday), workdt−1日)`、year_produ 補 `parseInt`；`Stage1FilterChain`；**⚠️ 改變 production 月名單分派各類名單案件數、無 flag 直接生效**） | AD-E07-22/25/26 / US-134 | P0-MVP（**v2.0**）|
| F092 | [F092-stage1-dry-run-estimate.md](features/F092-stage1-dry-run-estimate.md) | **Phase 3 — Stage 1 完整鏈 Dry-run 精確估算**（per-list estimate / F088 物化升級為完整鏈唯讀 dry-run COUNT ≡ 月名單分派；**v1.1**：同步 F091 v2.0 修正後 trigger / 去重上界 + 月名單分派落點 `ob_monthly_run_result`；唯讀不寫表） | AD-E07-23 / US-135 | P0-MVP（**v1.1**）|
| **F094** | [**F094-monthly-run-result-table.md**](features/F094-monthly-run-result-table.md) | **單源化 Phase A — 月名單分派結果表 `ob_monthly_run_result`**（migration `1711360000292`；PK=run_id+list_no+orgno+appl_no、FK→assignment_run CASCADE、nullable assignday；月名單分派 Stage 1 寫入 + Stage 3/4 讀取由 `ob_pool_data_list` 切換至本表；同一 PR 完整切換；snapshot 雙軌短期保留；**⚠️ Phase A 同批 deploy，結構切換不改案件數但須完整回歸**） | AD-E07-25 | P0-MVP（**v1.0 新增**）|
| **F095** | [**F095-applied-special-rules-readonly.md**](features/F095-applied-special-rules-readonly.md) | **特例規則前端唯讀呈現 — `appliedSpecialRules[]`**（名單詳情 API 讀時推導 list_nm → 規則清單，**無新 DB 欄位**；前端唯讀資訊區塊「此名單套用之系統特例規則」；trigger 判斷與 F091 v2.0 共用 pure utility；**prototype 落差待補**；Phase A 同批 deploy，唯讀不改月名單分派） | AD-E07-26 §26.5 | **P1**（**v1.0 新增**）|
| **F096** | [**F096-pooldata-whitelist-list-type-cleanup.md**](features/F096-pooldata-whitelist-list-type-cleanup.md) | **白名單清理 Phase B — `pooldata_field_whitelist.list_type` 停用**（migration/seed `1711360000293` 設 `is_active=false`；available-columns dropdown 不再顯示；澄清 `case_status → ob_pool_data.list_type` 為唯一期別篩選路徑；不改月名單分派案件數） | AD-E07-26 §26.7 | **P1**（**v1.0 新增**）|
...
#### M04 — 月名單分派執行模型重構（AD-E07-28：Worker 抽離 + Stage 1~4 SQL 下推）

> **v3.9 / 2026-06-02 / AD-E07-28（architecture-spec §5.13 v2.22，status: proposed）**：月名單分派由「cdmp-api 同程序 `setImmediate` 背景跑」重構為「pg-boss 入列 → 獨立 cdmp-worker 容器 → Stage 1~4 set-based SQL `INSERT…SELECT`」，分 P1/P2/P3 三階段。解決 (F1) event loop 阻塞（月名單分派期間整站 API 逾時）+ (F2) OOM（全載 ob_pool_data 進 heap）。**佇列 = pg-boss（免 Redis）；新增 cdmp-worker 容器；I-RUN-EST-01 estimate≡run 共用 buildStage1Sql；JS↔SQL PG 真庫等價測試為 P2/P3 DoD。** 建議實作順序 P1→P2→P3 嚴格序列。

| Feature ID | 文件 | 標題 | 來源 | 優先級 |
|------------|------|------|-----------|--------|
| **F098** | [**F098-monthly-run-worker-extraction.md**](features/F098-monthly-run-worker-extraction.md) | **P1 — 月名單分派 Worker 抽離**（pg-boss 入列 + `cdmp-worker` 容器 + `triggerRun` 改入列立即回 202 + `CancellationPoller` 補齊取消 + `OrphanReaper` 殭屍回收；解 F1；retryLimit=0 / 單 worker 序列化 / 不新增 orphan 欄位；I-TRIGGER-01；無 HTTP 錯誤碼） | AD-E07-28 P1 | P0-MVP（**v1.0 新增**）|
...
| F109 | [F109-customer-source-filter-fields.md](features/F109-customer-source-filter-fields.md) | 新增「客戶資料」來源篩選欄位（**v1.0 / 2026-07-02 / US-172**）：白名單新增 `data_source` 概念（`ob_pool_data` 案件資料 / `customer_core` 客戶資料，既有 7 筆預設 `ob_pool_data`）+ API 暴露 `dataSource` + M06 列表「資料來源」欄 + 名單定義「新增條件」選單依來源分組；新增 8 個 `data_source='customer_core'` 欄位（性別 code→label / 年齡衍生 AGE 以 `project_workym` 月首日為基準 / 居住城市 `LEFT(cpost_city,3)` 縣市級 22 / 5 個 `_desc` value=label）；F076 seed 7 個 categorical 欄位可選值（3/55/8/5/4/9/22）；月名單分派 Stage 1 條件式 LEFT JOIN customer_core（`custo_no=source_customer_no`）+ NULL 排除語意（BR-2/BR-3）+ 三處消費一致（月名單分派/Stage 0 試算/名單試算，BR-10）。5 個架構 OQ（OQ-F109-01~05）交 system-architect（AD-E07-37）。 | US-172 | P1 | **v1.0** |
| F112 | [F112-auto-suggest-categorical-options.md](features/F112-auto-suggest-categorical-options.md) | 類別型篩選欄位可選值自動建議（**v1.0 / 2026-07-12 / US-178**）：從來源表（`ob_pool_data` / `customer_core`）查 distinct 值批次帶入可選值之 UX 強化。新增 `GET /pooldata-fields/:columnName/distinct-values`（來源表解析 by `data_source` + `SAFE_COLUMN_NAME_RE` + INFORMATION_SCHEMA 存在性 + 就緒 503 + dialect-branch DISTINCT TOP CAP(200)+1 + truncation + `alreadyOption` 去重 + 逾時 504）與 `POST /pooldata-fields/:columnName/options/bulk`（單一 transaction + `display_order` max+1 依序 + 冪等略過 + 單一彙總稽核）。兩進入點：F075 新增欄位 Modal（僅 ob_pool_data，儲存編排建欄位→bulk）+ F076 可選值管理頁（含 customer_core，去重批次帶入）。18 AC / 14 TC；US-178 5 OQ 全裁示（cap 200 常數 / 沿用 tedious 逾時 / 兩次呼叫編排+非阻斷警告 / 稽核彙總一筆 / OQ-178-05 backlog）；新增 4 error code（`SOURCE_COLUMN_NAME_INVALID` 400 / `SOURCE_COLUMN_NOT_FOUND` 404 / `CUSTOMER_CORE_NOT_READY` 503 / `DISTINCT_VALUES_QUERY_TIMEOUT` 504）。3 架構 OQ（OQ-F112-01~03）交 system-architect（AD-E07-46）。 | US-178 | P2 | **v1.0** |
...
| [diagrams/F061-assignment-run-flow.mmd](diagrams/F061-assignment-run-flow.mmd) | 月名單分派 Stage 0~4 執行引擎流程 | Flowchart | F061 |
...
| [diagrams/F088-ready-summary.mmd](diagrams/F088-ready-summary.mmd) | **F088 / F089 v1.0 準備完成階段查詢摘要與 Rollback 資訊架構（三角色 × 四區塊 + 月名單分派前置條件耦合 + monthlyRunReady 即時更新）** | Flowchart | F088, F089, F061 v1.1, F082 v1.1 |
| [diagrams/F098-worker-extraction-flow.mmd](diagrams/F098-worker-extraction-flow.mmd) | **F098 v1.0 月名單分派 Worker 抽離 run 業務狀態機（pending→running→completed/failed，含 cancelRun / OrphanReaper 轉移來源 + I-TRIGGER-01）** | State | F098, F061 |
...
| [diagrams/F119-categorical-operator-flow.mmd](diagrams/F119-categorical-operator-flow.mmd) | ⚠️ **DRAFT** — **F119 v1.0 類別型文字比對運算子流程**：`operator` 解讀（BR-11 單一 fallback，缺漏 ≡ `in`）→ 文字形態關鍵字字面值跳脫（BR-7）→ 依 `data_source` 分派三個 fragment 建構器（`buildCategoricalFragment` 為 `ob_pool_data` 唯一落點，BR-4）→ **NULL 語意分流**（八格中唯一例外＝`ob_pool_data` × `not_contains` 保留 NULL；客戶兩來源沿用 `I-CC/CF-NULL-EXCLUDE-01` 全排除且禁特判）→ WHERE fragments 扇出至**五條執行路徑**（AC-14 依建構即一致）；另含重複判定簽章子圖（`:cat:` vs `:catop:` 前綴互斥、向後相容）。**未經 mermaid 工具驗證**（本機無 CLI） | Flowchart | F119, F050, F109, F075 |
...
3. E07 特別注意：NFR-003 月名單分派執行效能、NFR-004 快照原子性、NFR-005 結果準確性（F067 為主驗收工具）
...
F077 ──> F050 v2.1（新建名單預設 stage = 'draft'）、F052 v2.0（停用僅在草稿階段）、F061（月名單分派前置條件 stage = 'ready'）、F078（草稿推進）、後續批次 4+ Rollback / 簽核 spec
...
F086 ──> F061 v1.1（核准後 stage = 'ready'，月名單分派前置條件 BR-6 達成）
...
F088 ──> F061 v1.1（monthlyRunReady.allReady 為月名單分派前置條件 1 之核心入口）
...
F057 ──> F058（編輯需先查看）
F048, F050, F054, F055, F056, F058, F059, F060 ──> F061（月名單分派前置條件）
...
| 2026-04-24 | 新增 E07 客戶名單分派 21 個 Feature（F048~F068）+ 5 個圖表（F049/F061 flow+states/F066/F067）；新增 ASSIGNMENT 領域錯誤碼區段（LIST_NO_LIMIT_EXCEEDED / LIST_NO_DUPLICATE / ASSIGNMENT_RUN_ALREADY_RUNNING 等 20+ 項）；新增 NFR-003（月名單分派執行效能 < 30 min）、NFR-004（快照原子性 ACID）、NFR-005（分派結果準確性 < 3%，F067 為主驗收工具）；更新 scope/overview/open-questions；F067 優先級由 Should Have 升級為 P0-MVP；Feature 順序重排為 E01→E02→E03→E04→E05→E04/E05 跨模組→E06→E07；P0-MVP 增至 58 個；文件總數 104 份 | Spec Writer Agent |
...
| 2026-07-09 | **新增 F110（Code Decode 泛用 ETL 節點，US-173）**：新建 F110（E05）— 轉換節點 `code_decode`，為 `lookup`（F043 §4.8）之泛用化（同一字典表、單趟 N 組 mapping 一次 LEFT JOIN 解碼，取代一組各自全表 UPDATE 的 lookup）；config schema 節點級共用字典來源 + per-mapping matchColumn/lookupMatchColumn/filter/outputColumns（欄位名沿用 lookup）；固定 LEFT JOIN/NULL、TRIM/cast/重複 key 取首筆逐一對齊 lookup；逐格等價契約（US-173 AC-2）+「N lookup ⇔ 1 code_decode」雙向決定性對應；裁定 spec 層兩問題（單一 mapping 允許/customer_core 一律採用、filter=同 lookupFilter 自由式）；informative 附錄 customer_core 5 字典實例→9 節點/31 mapping。F043 v1.3→v1.4（§1 8→9 種、新增 §4.9 摘要、§6 邊界、§8 相關文件）。無新錯誤碼、無新圖表。E05 增至 18、P0-MVP +1；文件總數 154 份 | Spec Writer Agent |
