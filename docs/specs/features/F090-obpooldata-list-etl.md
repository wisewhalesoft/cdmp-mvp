...
> 1. **`data_source` 值域單源化**（DP-AD25-1 方案 A）：值域由 `'etl_legacy'` / `'monthly_run'` 雙值改為**單一值 `'etl_load'`**。`ob_pool_data_list` 自此**只承載 ETL 載入之 legacy 派案歷史**，不再混入本系統月跑提案；月跑提案結果改寫入新表 **`ob_monthly_run_result`**（見 [F094](F094-monthly-run-result-table.md)）。migration `1711360000291` 已存在（v1.0 建立），**v2.0 不新建 migration，僅更新應用層值域說明 + entity 註記**。
> 2. **ETL Load 刪除邊界放寬**（AD-E07-25 §25.3）：因本表不再有月跑資料需保護，`E07-OBPOOLDATA_LIST-Load` 之前置 DELETE 可改全量 `DELETE FROM ob_pool_data_list`（或維持 `DELETE WHERE data_source='etl_load'`，二者等效，因表內僅此一值）。
> 3. **既有 `monthly_run` / NULL 資料自然淘汰**（DP-AD25-5）：不需額外 data migration；待下次 ETL 全量覆寫自動清除。
>
> **Phase 對應**：`data_source` 值域 + 去重來源單源化屬 **Phase B**（[AD-E07-25 §25.7](../architecture-spec.md)）；月跑寫入目標切換至 `ob_monthly_run_result` 屬 **Phase A**（見 [F094](F094-monthly-run-result-table.md)，與 [F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md) 同批 deploy）。本 feature 之 v2.0 變更（值域說明 + Load 邊界放寬）可隨 Phase A/B 任一批落地，因表語意以 F094 完成「月跑不再寫入本表」為前提。
>
...
> **v2.0 單源化（AD-E07-25）**：`ob_pool_data_list` 自 v2.0 起為 **ETL 單一來源**，只承載 legacy 派案歷史，`data_source` 值域單一化為 `'etl_load'`。本系統月跑提案結果改寫入 `ob_monthly_run_result`（[F094](F094-monthly-run-result-table.md)），不再與 ETL 歷史混存於本表（消除 v1.0 雙重角色設計之語意污染）。
...
- **And**（v2.0）本表不再有月跑提案資料需保護（月跑改寫 [F094](F094-monthly-run-result-table.md) `ob_monthly_run_result`），故無需 v1.0 之 per-`data_source` 保護性截斷；既有殘留 `'monthly_run'` / NULL 列由全量覆寫自然淘汰（DP-AD25-5）
...
### AC-5：月跑 Stage 1 寫入目標改為 `ob_monthly_run_result`（v2.0 單源化）

- **Given** 月跑 Stage 1（`AssignmentRunPipelineService`）寫入本月分派提案
- **When** 月跑為某 `list_no` 寫入本月分派結果
- **Then**（**v2.0 變更**）月跑寫入目標改為 **`ob_monthly_run_result`**（見 [F094](F094-monthly-run-result-table.md)），**不再寫入 `ob_pool_data_list`**（AD-E07-25 §25.2）
- **And** `ob_pool_data_list` 自此為純 ETL 單一來源（僅 `data_source='etl_load'`），不含月跑提案
...
> **Phase 邊界提示**：AC-5 之「月跑寫入目標切換至 `ob_monthly_run_result`」屬 **Phase A**（資料落點切換，與 [F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md) 同批 deploy），其完整資料契約（schema / pipeline 寫入 / Stage 3/4 讀取切換）見 [F094](F094-monthly-run-result-table.md)。本 feature（F090）之 ETL 載入與去重來源單源化屬 Phase 1 / Phase B；F090 ETL 本身不影響 production 月跑案件數（去重讀取行為屬 F091 Phase 2）。
...
| Load 前置 DELETE | **（v2.0）** 全量 `DELETE FROM ob_pool_data_list`（或等效 `DELETE WHERE data_source='etl_load'`）；本表不再有月跑提案需保護（AD-E07-25 §25.3） |
...
| BR-2 | **單一來源（v2.0，AD-E07-25 §25.2）**：`ob_pool_data_list` 自 v2.0 起**僅承載 ETL 歷史派案紀錄**（去重查詢用），`data_source` 值域單一化 `'etl_load'`。本系統月跑提案結果改寫入 `ob_monthly_run_result`（[F094](F094-monthly-run-result-table.md)），不再混存於本表（消除 v1.0 雙重角色之語意污染）|
| BR-3 | **ETL Load 不可全表 TRUNCATE（但前置 DELETE 可全量）**：`E07-OBPOOLDATA_LIST-Load` 之 `fullMode` 仍應為 `false`（避免引擎層 TRUNCATE 重建表結構/索引）；前置 DELETE 自 v2.0 可全量 `DELETE FROM ob_pool_data_list`（或 `DELETE WHERE data_source='etl_load'`，等效），因本表不再有月跑提案需保護 |
| BR-4 | **月跑寫入目標切換（v2.0）**：月跑 Stage 1 寫入目標改為 `ob_monthly_run_result`（[F094](F094-monthly-run-result-table.md)），不再寫入 `ob_pool_data_list`；本表不再被月跑寫入 |
...
| BR-6 | **ETL 不於月跑進行中執行**：ETL（全量 DELETE + INSERT）不應與月跑並發，避免讀寫干擾；月跑前手動完成 ETL（§9） |

## 7. 錯誤場景

| 場景 | 系統回應 | 參考 |
|---|---|---|
| ETL 載入與月跑同時操作 | 本設計以「歷史限定（`ASSIGNDAY < 本月第一天`）」+ v2.0 月跑改寫 `ob_monthly_run_result`（不再寫本表）消除衝突（BR-1 / BR-4）；理論上不再發生 | [AD-E07-25 §25.2](../architecture-spec.md) |
...
| ~~`'monthly_run'`~~ | ~~本系統月跑 Stage 1 寫入的本月分派結果~~ | ~~月跑（v1.0）~~ | **廢止（月跑改寫 `ob_monthly_run_result`，[F094](F094-monthly-run-result-table.md)）** |
...
| 月跑執行（Stage 1）| **（v2.0）** 寫入 `ob_monthly_run_result`（[F094](F094-monthly-run-result-table.md)），**不再寫入 `ob_pool_data_list`** |

> **注意**：ETL 不應於月跑進行中執行（避免讀寫干擾）。月跑前手動完成 ETL 後才啟動月跑（BR-6）。
...
- 架構決策：[architecture-spec.md AD-E07-25 v1.1](../architecture-spec.md)（`ob_pool_data_list` 單源化 + `data_source='etl_load'` + 月跑改寫 `ob_monthly_run_result`，**權威來源**）、[AD-E07-21 v1.1](../architecture-spec.md)（OBPOOLDATA_LIST ETL 原始設計）、[AD-E07-24 §24.2](../architecture-spec.md)（Phase 影響）
...
- 相關功能：[F094](F094-monthly-run-result-table.md)（月跑改寫之結果表）、[F091](F091-stage1-complete-month-cnt-dedup-special-delete.md)（去重來源）、[F092](F092-stage1-dry-run-estimate.md)、[F061](F061-trigger-assignment-run.md)（月跑觸發）
...
- **本 Phase（F090 ETL 建立 + 去重來源單源化）對 production 月跑分派案件數無影響**（[AD-E07-24 §24.2](../architecture-spec.md)）：F090 僅建立 ETL 與單源化 schema 語意；去重讀取行為屬 [F091](F091-stage1-complete-month-cnt-dedup-special-delete.md) Phase 2。
- **v2.0 之月跑寫入目標切換**（`ob_pool_data_list` → `ob_monthly_run_result`）屬 **Phase A** 結構變更，其 production 影響見 [F094 §13](F094-monthly-run-result-table.md)（與 [F091 v2.0 §13](F091-stage1-complete-month-cnt-dedup-special-delete.md) 同批 deploy）。
- 改變 production 月跑案件數的階段為 **F091（Stage 1 補完整 + 特例規則 SP 修正）**，依 DP-AD23-2 / DP-AD26-1 **無 feature flag、deploy 後直接生效**（詳見 [F091 §13](F091-stage1-complete-month-cnt-dedup-special-delete.md)）。
