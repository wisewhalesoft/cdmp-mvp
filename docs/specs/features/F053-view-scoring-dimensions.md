...
提供業務部長 / 業務處長查看目前選中 CARD_TYPE(由 F069 Tab 1 維護)之生效計分版本(`ob_levelcard_version` 中 `card_type = :selectedCardType AND status = 'active'`)的計分維度清單與各維度詳細分數表。純唯讀查看;寫入操作由 F054 負責。本功能屬 M02 計分設定 5 Tab 結構中的 Tab 2。
...
- `businessRole IN ('director','section_chief')`（M02 計分卡 GET 端點開放部長 + 處長，後端套用 `DirectorOrSectionChiefGuard`，依 F002 §4.6.2）
...
- **Given** 業務部長 / 業務處長已在 Tab 1 選中某 CARD_TYPE，並切換至 Tab 2
- **When** Tab 2 載入完成
...
- **Given** 業務部長 / 業務處長已在 Tab 2 查看某 CARD_TYPE 的計分維度
- **When** 業務部長 / 業務處長切換回 Tab 1 選中不同 CARD_TYPE，再切回 Tab 2(或直接於 Tab 1 點擊另一列觸發 Tab 2 reload)
...
- **Given** Tab 1 之 CARD_TYPE 清單為空，或業務部長 / 業務處長尚未在 Tab 1 選中任何一筆
- **When** 業務部長 / 業務處長切換至 Tab 2
- **Then** 顯示提示：「請先在 Tab 1 選擇計分卡類型以查看設定」
- **And** API 不被呼叫(前端不送出 request)

### AC-7：CARD_TYPE 不存在於 active scope

- **Given** 業務部長 / 業務處長送出之 `cardType` query 不存在於 `ob_card_type` 之 active 紀錄(如已被 F072 停用後 race condition)
- **When** 後端查詢
...
**Controller 規範**：使用 `DirectorOrSectionChiefGuard` + `@RequireDirectorOrSectionChief()`（依 F002 §4.6.2，M02 計分卡讀取為 GET 端點，部長 + 處長皆可）。
