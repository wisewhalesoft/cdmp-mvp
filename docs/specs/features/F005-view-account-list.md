...
Admin 可查看所有使用者帳號的分頁清單，包含搜尋與篩選功能。此清單為帳號管理操作（編輯、停用/啟用、角色變更、密碼重設）的主要入口。
...
- When 頁面載入完成
- Then 系統顯示分頁帳號清單，每筆包含：姓名、Email、角色、狀態（active / disabled）、建立日期（created_at）
...
| 項目 | 說明 |
|------|------|
| 清單欄位 | 姓名、Email、角色、狀態、建立日期、操作（編輯/停用/角色變更/重設密碼） |
| 搜尋 | 搜尋框位於清單上方，支援即時搜尋或按鍵搜尋 |
| 篩選 | 角色篩選（全部/Admin/User）與狀態篩選（全部/啟用/停用） |
