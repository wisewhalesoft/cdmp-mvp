---
spec-id: F094
title: 月名單分派結果表 ob_monthly_run_result（單源化 Phase A：pipeline 落點切換）
feature-id: F094
...
# F094: 月名單分派結果表 ob_monthly_run_result（單源化 Phase A）

Priority: P0-MVP | Status: Draft | Last Updated: 2026-05-27

> ⚠️ **PRODUCTION 結構變更警告（必讀）**：本 feature 將月名單分派 Stage 1~4 之寫入 / 讀取目標由 `ob_pool_data_list`（`data_source='monthly_run'`）切換至新表 `ob_monthly_run_result`。依 [AD-E07-25 §25.7 Phase A](../architecture-spec.md)，**月名單分派寫入目標改變需於同一 PR 完整切換（Stage 1 寫入 + Stage 3/4 讀取），不可分批**，與 [F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md)（特例規則 SP 修正）+ [F095](F095-applied-special-rules-readonly.md)（前端唯讀 API）同批 deploy（Phase A）。切換本身不改變月名單分派「案件數」（案件數由 F091 改變），但改變資料落點與下游讀取路徑，**deploy 前須完成回歸驗證**（見 §13）。
>
> **v1.0（2026-05-27 / AD-E07-25 單源化 Phase A）**：依 [architecture-spec.md AD-E07-25 v1.1](../architecture-spec.md)（全 DP Resolved）落地。新建 `ob_monthly_run_result` 表（migration `1711360000292`），承載「本次月名單分派對各名單的分派提案」（Stage 1 寫入 → Stage 2 計分 → Stage 3 CR → Stage 4 分派）；月名單分派 pipeline 寫入目標與 Stage 3/4 讀取目標由 `ob_pool_data_list` 切換至此表，使 `ob_pool_data_list` 回歸 ETL 單一來源（見 [F090 v2.0](F090-obpooldata-list-etl.md)）。
>
...
新建 `ob_monthly_run_result` 表作為月名單分派提案的結構化落點，承載 Stage 1~4 之計算結果（案件識別、計分、CR、分派部門/業務員），以 `run_id` FK 關聯 `assignment_run`（ON DELETE CASCADE）。月名單分派 pipeline 之寫入目標（Stage 1）與讀取目標（Stage 3/4）由 `ob_pool_data_list`（`data_source='monthly_run'`）切換至本表。此切換使 `ob_pool_data_list` 回歸「ETL 單一來源」語意，消除 [AD-E07-25 §25.1](../architecture-spec.md) 所列雙重角色設計之結構性問題（語意污染、ETL 刪除邊界隱性依賴）。

## 2. 使用者故事

**As a** 系統架構維運人員 / 分派維運人員
**I want** 月名單分派提案（本系統產出的候選清單）與業務系統歷史真相（ETL 載入的 `ob_pool_data_list`）分離儲存於獨立結果表
**So that** 去重查詢的來源語意清晰（只查 ETL 歷史）、月名單分派結果可依 `run_id` 整批追蹤與級聯清除，且未來業務回填狀態可結構化記錄

## 3. 前置條件

- `assignment_run` 表已存在（`run_id` UUID PK，FK 目標）
- migration m111（`ob_pool_data_list`）已存在
- 既有月名單分派 pipeline（`AssignmentRunPipelineService.runPipeline()` / `executeV1()` / `executeV2()`）可用，其 Stage 1 寫入與 Stage 3/4 讀取目前指向 `ob_pool_data_list`
...
- **And** FK `fk_omrr_run` → `assignment_run(run_id)` `ON DELETE CASCADE`（月名單分派 run 刪除時自動清除對應結果列）
...
### AC-2：月名單分派 Stage 1 寫入目標切換至 `ob_monthly_run_result`

- **Given** 月名單分派 Stage 1 為某 `list_no` 完成案件挑選（[F091 v2.0 `executeStage1Chain({ dryRun: false })`](F091-stage1-complete-month-cnt-dedup-special-delete.md)）
- **When** 寫入本月分派提案
...
### AC-3：月名單分派 Stage 3/4 讀取目標切換至 `ob_monthly_run_result`

- **Given** 月名單分派 Stage 3（CR 回分）/ Stage 4（部門 / 業務員分配）需更新分派結果
...
- **And** 月名單分派 Stage 4 完成後仍寫 `assignment_run_snapshot`（type=result）作為稽核快照（與 `ob_monthly_run_result` 並存）
...
- **When** 本 feature 切換月名單分派落點至 `ob_monthly_run_result`
...
| 欄位群組 | 欄位 | 說明 |
|---|---|---|
| PK / 識別 | `run_id`(UUID) / `list_no`(VARCHAR100) / `orgno`(VARCHAR2) / `appl_no`(VARCHAR10) | 月名單分派 ID + 名單 + 機構 + 案件申請號 |
...
| `idx_omrr_list_run` | `(list_no, run_id)` | 按名單查某次月名單分派結果 |
...
| 規則編號 | 說明 |
|---|---|
| BR-1 | **月名單分派寫入單一落點**（AD-E07-25 §25.2）：月名單分派 Stage 1~4 提案結果寫入 `ob_monthly_run_result`，不再寫入 `ob_pool_data_list` |
| BR-2 | **精簡欄位 + JOIN 取業務欄位**（DP-AD25-2）：本表僅存 Stage 2~4 計算結果，不複製 `ob_pool_data` 全部業務欄位；計算時直接用 in-memory `ObPoolData[]`，無需額外 JOIN |
| BR-3 | **run 級聯清除**：`run_id` FK + `ON DELETE CASCADE`，月名單分派刪除時自動清除對應結果列（AC-6） |
...
| BR-7 | **result_status 初始 PENDING**：月名單分派寫入時 `result_status='PENDING'`；業務系統回填後改 `'SUCCESS'`/`'FAILED'`（回填流程屬 Phase C，本輪不交付）|
...
| 月名單分派 run 重跑（同 list_no 重複寫入）| 由 pipeline 寫入策略處理（per-run_id 隔離 / 寫入前清除同 run_id 之列）；不同 `run_id` 天然隔離，PK 含 `run_id` 不衝突 | [F061](F061-trigger-assignment-run.md) |
| `assignment_run` 刪除 | FK CASCADE 自動清除本表對應列（AC-6），無孤兒列 | — |

## 8. 相依性

- **Blocked By**：`assignment_run` 表、既有月名單分派 pipeline（`AssignmentRunPipelineService`）
- **Blocks**：[F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md)（Stage 1 鏈回傳型別 `Partial<ObMonthlyRunResult>[]` 依賴本表）
- **配套**：[F090 v2.0](F090-obpooldata-list-etl.md)（`ob_pool_data_list` 單源化，月名單分派不再寫該表）
...
- 相關功能：[F090 v2.0](F090-obpooldata-list-etl.md)、[F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md)、[F061](F061-trigger-assignment-run.md)（月名單分派觸發）、[F062](F062-view-run-progress.md)（執行進度）
...
- 關鍵測試案例：
  ...
  - Stage 1 寫入：月名單分派寫入本表（非 `ob_pool_data_list`），`result_status='PENDING'`，回傳型別 `Partial<ObMonthlyRunResult>[]`
  - Stage 3/4：讀取與更新本表（`is_cr`/`cr_id`/`cr_nm`、`dept_id`/`emplid`/`emplid_deptid`）
  - **回歸（斷鏈防護）**：Stage 1 寫入 + Stage 3/4 讀取均指向本表（同一表）；月名單分派完整鏈 e2e 通過
  - **去重來源**：Stage 1 去重查詢只讀 `ob_pool_data_list`，不讀本表（與 F091 AC-2 對齊）
  - snapshot 雙軌：月名單分派仍寫 `assignment_run_snapshot` type=result；`collectCrCandidates()` 仍讀 snapshot
...
| A-2 | 月名單分派重跑時同 `run_id` 之寫入冪等策略（寫入前清除同 run_id 列 vs 依賴新 run_id 天然隔離）由 tdd-implementation 依既有 pipeline 冪等慣例決定 | [ASSUMPTION] |
...
- **本 feature 改變月名單分派提案之資料落點與下游讀取路徑**（`ob_pool_data_list` → `ob_monthly_run_result`），屬 **Phase A** 結構切換，與 [F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md) + [F095](F095-applied-special-rules-readonly.md) 同批 deploy。
- **本切換不改變月名單分派「案件數」**（案件數由 [F091 v2.0](F091-stage1-complete-month-cnt-dedup-special-delete.md) 之 Stage 1 篩選邏輯改變）；本 feature 改變的是「資料存放位置」與「Stage 3/4 讀取來源」。
- **原子性風險（AC-4 / BR-5）**：Stage 1 寫入與 Stage 3/4 讀取**必須同批切換**；部分切換將導致下游讀不到提案（資料斷鏈）。**deploy 前須完成月名單分派完整鏈 e2e 回歸驗證**（Stage 1→2→3→4 全程讀寫本表）。
- **回歸驗證範圍（建議）**：
  1. 月名單分派完整鏈 e2e：確認 Stage 1 寫入本表 → Stage 2 計分 → Stage 3 CR → Stage 4 分派，全程讀寫 `ob_monthly_run_result`。
  2. 去重來源：確認 Stage 1 去重仍只讀 `ob_pool_data_list`（不誤查本表）。
  3. snapshot 雙軌：確認 `collectCrCandidates()` 仍讀 snapshot、月名單分派仍寫 type=result 快照。
