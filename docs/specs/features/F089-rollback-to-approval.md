...
> 4. **本 v1.3 不變更既有業務邏輯**（API endpoint / 設定資料全部保留不清空 / Transaction 原子性 / 月跑鎖 / 稽核 / Feature Flag）；僅入口位置與成功 toast 行為變更。
>
...
- **And** Kanban `ready` 卡片**不渲染** per-card「觸發月跑」按鈕（依 [F061 v1.4 AC-No-Per-Card-Trigger](F061-trigger-assignment-run.md)；月跑唯一入口為 Ready 欄頂 CTA Banner）
...
- **`ready` 卡片不渲染 per-card 觸發按鈕**：依 [F061 v1.4 AC-No-Per-Card-Trigger](F061-trigger-assignment-run.md)；月跑唯一入口為 Ready 欄頂 CTA Banner
...
- F088 頁面之「月跑前置條件提示」依本 Rollback 動態重新計算
