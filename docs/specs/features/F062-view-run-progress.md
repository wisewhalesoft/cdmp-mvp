...
提供業務部長 / 業務處長在月名單分派執行期間即時查看各 Stage 的執行進度。採前端 Polling（預設每 3 秒輪詢一次，與 AD-E07-2 一致）。月名單分派完成顯示結果入口（連至 F063 / F064）；月名單分派失敗顯示錯誤訊息與重新觸發按鈕（重新觸發限部長，見 F061 §4.6）。
...
- **Given** 月名單分派已觸發（F061），進度頁 URL 含 `run_id`
- **When** 業務部長 / 業務處長進入執行進度頁
- **Then** 顯示月名單分派資訊：`run_id`、`project_workym`、`triggered_by`（觸發者名稱）、`triggered_at`（觸發時間）、目前 `status`、已執行時間
...
- 月名單分派完成後：顯示 CTA 連結至 F063 / F064 / F066
- 月名單分派失敗：紅色警示 + `error_message` + 「重新觸發」按鈕

## 8. 相依性

- **Blocked By**：F061（月名單分派已觸發才有 `run_id`）
- **Blocks**：無（進度頁為獨立查看功能）
