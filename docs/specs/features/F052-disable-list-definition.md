...
提供業務主管停用名單定義功能（軟刪除）。停用為不可逆操作（MVP 範圍內不提供重新啟用）；停用後 `status` 由 `'active'` 更新為 `'inactive'`，資料不刪除，歷史快照不受影響。月跑執行中禁止停用。
...
- **Given** 業務主管在 F048 清單頁點擊某個 `status = 'active'` 名單的「停用」按鈕
- **When** 系統彈出確認對話框
...
- **And** 後端不提供對應 API 端點；若業務需恢復使用相同條件，業務主管可透過 F050「複製名單」建立新名單
...
| BR-2 | 不提供重新啟用（MVP 範圍外）；業務主管需透過 F050「複製名單」建立新紀錄 |
