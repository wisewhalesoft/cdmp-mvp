...
- 月份切換：可選範圍前後 12 個月（共 25 月）；歷史月份為唯讀；目前作業月份完整操作；未來月份允許預先建立草稿（部長 / Admin 限定）。**預設選取為「分派作業月份」`target_work_ym`（= `current_work_ym + 1`，下月；v1.4 / F097）**，由前端 `AssignmentWorkYmContext` 四頁共享（[F097 §5.1](F097-work-ym-semantics-unification.md)）
...
- **And** 預設顯示分派作業月份（`target_work_ym`，= `current_work_ym + 1`，即下月；v1.4 / F097）
...
- **Given** 帳號持有「處長」角色
- **When** 進入清單頁
- **Then** 僅顯示本處長轄區（後端依 `SectionChiefScopeService.getScopeDeptCode(userId)` 反查 dept_code 後以 `ob_dept_pct` EXISTS 過濾，對齊 BR-4 v1.4 / BR-7 C-4）的名單；其他處長轄區不顯示（**v1.4 / F097 順修**：廢除 v1.3 殘留之「`created_by = currentUserId` 過濾」舊文字——chicken-and-egg：處長不會建名單，永遠 0 match）
...
> ⚠️ **既有技術債 note（v1.4 / F097 / OQ-F097-01 方案 A）**：本表 `INVALID_YM_RANGE` / `INVALID_YM_FORMAT`（400）之碼名與 HTTP status 與 [error-handling.md](../error-handling.md) 既有登記之 `WORK_YM_OUT_OF_RANGE` / `WORK_YM_INVALID_FORMAT`（**422**）**雙重不一致**（碼名 + status）。F097 採方案 A **不於本輪統一此塊**（避免擴大範圍），新實作（如 F097 `POST /runs` `workYm` 驗證）一律以 error-handling.md 之 `WORK_YM_INVALID_FORMAT`（422）為準。**未來 cleanup（OQ-F097-01 選項 C）**：全 E07 `ym` 格式 / 範圍碼統一正名 + status 對齊，需 system-architect 介入，列為 follow-up。
...
- 前端關鍵測試案例：
  - 月份切換器預設值 = `target_work_ym`（= `current_work_ym + 1`，下月；v1.4 / F097）；跨年邊界（`current_work_ym = '202512'` → 預設 `'202601'`）
  - 月份切換器 label = 「分派作業月份」；無「作業年月 / 當月 / 本月」字串
