---
spec-id: F102
title: 月跑 CR 優先分派（失效清空 + CR 優先指派 + 扣量 + per-list cr_enabled 閘控 + 廢除全域旗標）
feature-id: F102
...
- **閘控（US-153）**：月跑於 Stage 3 前置處理逐名單讀取 `ob_list_definition.cr_enabled` 快照值；`true` 才執行 CR 前置步驟，`false` 則跳過並將該名單所有案件 `is_cr` 強制為 `'N'`，全部進入 F101 標準比例分派池。
...
- **純後端**：F102 為月跑 pipeline 內前置處理，**無新前端頁**。

## 2. 使用者故事

**As a** 業務主管
**I want** 月跑在 F101 Stage 3/4 比例分派之前，依各名單 `cr_enabled` 設定，先清空失效的 CR 標記（逾2年或業代離職）、將有效 CR 案件優先指派回原 CR 業代，並讓後續比例分派只處理尚未被 CR 預指派的案件
**So that** CR 客戶（曾被特定業代服務的歷史客戶）能回到原業代手中，月跑結果之 CR 三欄（`cr_id` / `cr_nm` / `is_cr`）有值，對齊 legacy 名單約 1.9% 的 CR 案件比例，且不同名單可獨立決定是否啟用 CR
...
- `ob_list_definition.cr_enabled`（BOOLEAN NOT NULL DEFAULT false；migration `1711360000182`）有該名單設定值（月跑開始時快照，US-153 AC-4）。
...
**BR-F102-01（per-list 閘控）**：月跑於 Stage 3 前置處理逐名單讀取 `ob_list_definition.cr_enabled`（**月跑開始時之快照值**，與 F101 其他名單參數 `ob_dept_pct` / `ob_empl_set` 快照時機一致，US-153 AC-4）。`cr_enabled=true` → 執行步驟 1–3；`cr_enabled=false` → 跳過步驟 1–3。

**BR-F102-02（停用名單 is_cr 強制清 N）**：`cr_enabled=false` 之名單，其工作集所有案件 `is_cr` 強制為 `'N'`（執行 `UPDATE ob_monthly_run_result SET is_cr='N' WHERE list_no=:listNo AND run_id=:runId AND (is_cr IS NULL OR is_cr<>'N')`），避免來源 `ob_pool_data_list` 殘留之 `is_cr='Y'` 污染月跑結果；該名單所有案件進入 F101 標準比例分派池，不扣量。
...
- **Given** 名單 OB202606001 之 `ob_list_definition.cr_enabled=true`；月跑觸發、Stage 2 已完成
- **When** 月跑進入 Stage 3 前置 CR 處理
- **Then** 為 OB202606001 執行步驟 1（逾2年清空）、步驟 2（離職清空）、步驟 3（CR 優先指派）
- **And** 月跑執行日誌記錄「OB202606001：cr_enabled=true，執行 CR 優先分派」（BR-F102-01）
...
- **Given** 同一月跑含 OB202606001（`cr_enabled=true`）與 OB202606002（`cr_enabled=false`）
- **When** 月跑完整執行（Stage 0–4）
- **Then** OB202606001 有 `is_cr='Y'`、`emplid=cr_id` 之案件；OB202606002 所有案件 `is_cr='N'`、走標準比例分派
- **And** 兩份名單之 Stage 3/4 比例分派互不干擾，月跑整體 `status='completed'`（BR-F102-01/02）

#### AC-4：cr_enabled 月跑開始後鎖定（US-153 AC-4）

- **Given** 月跑開始時讀取 `cr_enabled` 快照
- **When** 月跑執行中（`status='running'`）管理員嘗試修改名單 `cr_enabled`
- **Then** 修改被月跑鎖阻擋（沿用 US-107 / US-104 鎖定機制）
- **And** 月跑全程使用開始時之 `cr_enabled` 快照值，不受後續變更影響（BR-F102-01）
...
**BR-F102-05（限工作集）**：清空操作只作用於本次月跑之 `ob_monthly_run_result` 工作集（`run_id` 限定），**不修改** `ob_pool_data_list` 原始資料。
...
- **Given** 相同月跑設定（相同 `list_no` / `work_ym` / `ob_empl_set` / `ob_emphire` / `cr_enabled` 資料），以不同 `run_id` 執行兩次
...
- **Given** 202606 月跑（含 `cr_enabled=true` 名單）重跑後，[F064](F064-export-assignment-result.md) 匯出 `ob_monthly_run_result`
- **When** 以 `is_cr='Y'` 過濾
...
F102 為**純後端月跑 pipeline 前置處理**，無新 HTTP API 端點、無新前端頁、無新錯誤碼。
...
- 閘控讀取時機：建議在月跑 Stage 3 前置處理之 per-list 迴圈開始時讀取，與 F101 Stage 3 比例分派之 per-list 讀取點一致（US-153 技術備註）。
