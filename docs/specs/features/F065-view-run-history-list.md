...
提供業務主管查看所有歷史月跑的執行紀錄清單，依 `triggered_at DESC` 排序（對應 `assignment_run.triggered_at DESC` 索引）。支援分頁、年月篩選、狀態篩選。僅顯示 `status IN ('completed', 'failed')` 的月跑（`pending` / `running` 顯示於 F062 進度頁）。
...
- **Given** 歷史清單已顯示
- **When** 業務主管使用篩選器（`projectWorkym` 下拉、`status` 下拉）
