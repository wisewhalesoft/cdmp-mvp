...
CDMP 是一套企業級客戶資料治理平台，旨在為組織內部團隊提供統一的資料來源管理能力。MVP 階段聚焦於七大核心能力：安全驗證與登入、帳號與角色管理、資料來源（資料庫連線）的設定與監控、資料擷取管理（從外部資料來源讀取資料並搬移至 CDMP AppDB）、ETL Pipeline 管理（透過視覺化編輯器組合 Extract / Transform / Load 節點，將 raw data 轉換後載入 Domain-Oriented 目標表）、Customer 360（跨來源客戶檢視）、以及客戶名單分派（Sales Manager 自助完成名單分派月名單分派全流程）。
...
- **角色定位**：一般平台使用者
- **職責範圍**：登入平台、自助式密碼重設、查詢 Customer 360（E06）
- **存取權限**：可存取 E01（驗證）、E06（Customer 360）；敏感欄位以遮罩呈現
- **典型場景**：客服人員、業務人員、其他部門使用者

### Sales Manager（業務主管）

- **角色定位**：User + `is_sales_manager = true`（由 Admin 設定）
- **職責範圍**：獨立完成客戶名單分派全流程（M01 名單定義、M02 計分設定、M03 分派比例、M04 分派執行、M05 快照歷史、M06 代碼維護），無需 IT 介入
- **存取權限**：E01 + E06 + E07 全部端點（`/api/v1/assignment/**`）；Admin 為超集（額外可存取 E02~E05）
- **典型場景**：每月名單分派作業負責人；負責調整計分參數、部門與人員比例、觸發月名單分派、檢視結果、匯出清單
...
- 登入 / 登出 / Session 管理
- 帳號 CRUD 與角色管理（含業務主管旗標 `is_sales_manager`）
- 資料來源 CRUD、連線測試、健康監控
...
- Customer 360（客戶搜尋、清單、360 詳情檢視）
- E07 客戶名單分派（M01 名單定義、M02 計分設定、M03 分派比例、M04 分派執行、M05 快照歷史、M06 代碼維護；ob_* 表遷移至 AppDB，月名單分派非同步執行，三份快照原子性寫入）
...
| E07 月名單分派完整執行時間 | < 30 分鐘（10 萬筆案件） | 整合測試 + 實際資料壓測（NFR-003） |
| E07 快照原子性 | 100% ACID（無部分寫入殘留） | DB 交易測試（NFR-004） |
| E07 分派結果準確性 | 人員配對不一致率 < 3% | 新舊系統並行驗證 + F067（NFR-005） |
...
6. 系統僅使用 Admin 與 User 兩種角色

## 關鍵限制

1. Phase 1 MVP 僅使用 2 種角色（Admin、User），均為系統預設 Seed Data，不支援自訂角色新增或刪除
2. 技術棧尚未決定 — 本規格書保持技術中立，不指定特定框架或資料庫引擎
3. 不提供帳號鎖定機制（登入失敗次數限制延後至後續版本）
