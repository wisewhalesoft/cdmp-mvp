...
CDMP 是一套企業級客戶資料治理平台，旨在為組織內部團隊提供統一的資料來源管理能力。MVP 階段聚焦於三大核心能力：安全驗證與登入、帳號與角色管理、以及資料來源（資料庫連線）的設定與監控。
...
4. **確保憑證安全** — 使用者密碼以 bcrypt 雜湊儲存，資料庫連線密碼以 AES-256 加密儲存
...
- **目標資料庫**：MySQL、PostgreSQL、SQL Server 實例（系統連線測試與健康檢查的對象）
...
- **Feature ID**：F001-F016 連續編號，對應 User Story ID
