...
CDMP 是一套企業級客戶資料治理平台，旨在為組織內部團隊提供統一的資料來源管理能力。MVP 階段聚焦於七大核心能力：安全驗證與登入、帳號與角色管理、資料來源（資料庫連線）的設定與監控、資料擷取管理（從外部資料來源讀取資料並搬移至 CDMP AppDB）、ETL Pipeline 管理（透過視覺化編輯器組合 Extract / Transform / Load 節點，將 raw data 轉換後載入 Domain-Oriented 目標表）、Customer 360（跨來源客戶檢視）、以及客戶名單分派（Sales Manager 自助完成名單分派月跑全流程）。
...
- **典型場景**：每月名單分派作業負責人；負責調整計分參數、部門與人員比例、觸發月跑、檢視結果、匯出清單
...
- E07 客戶名單分派（M01 名單定義、M02 計分設定、M03 分派比例、M04 分派執行、M05 快照歷史、M06 代碼維護；ob_* 表遷移至 AppDB，月跑非同步執行，三份快照原子性寫入）
...
| E07 月跑完整執行時間 | < 30 分鐘（10 萬筆案件） | 整合測試 + 實際資料壓測（NFR-003） |
