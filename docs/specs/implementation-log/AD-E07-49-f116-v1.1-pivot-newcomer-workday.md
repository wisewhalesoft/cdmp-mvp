...
version: "1.1"
date: 2026-08-13
status: approved（隨母 spec F116 v1.1.1 — 本 AD 無業務阻塞事項，A-1~A-5 皆為 HOW 層級裁定；v1.1 為同日職稱來源欄位更正）
...
>
> **v1.1 同日更正（2026-08-13）**：職稱來源欄位由 `ob_emphire.jfun_nm` 改為 **`ob_emphire.title_name`**；API 欄位 `jfunNm` → **`titleName`**（隨母 spec F116 v1.1.1 反轉 BR-5，理由與實測數據見 §10 變更紀錄）。本節以下 §2~§8 已同步改為 `title_name`/`titleName`；純欄位替換，**不影響** A-1~A-5 任何裁定，不需新增不變式。
...
v1.1 裁定改為 LEFT JOIN 一個 `emp_id` 去重 derived table，新增 `title_name` / `hire_date` 至 SELECT + GROUP BY（T-4 要求）：
...
const qb = repo
  ...
  .addSelect('r.emplid', 'emplid')
  .addSelect('e.emp_nm', 'empNm')
  .addSelect('e.title_name', 'titleName')       // v1.1 新增（AC-6）
...
「(空白)」節點（`e.emp_id IS NULL`）：`row.titleName`/`row.hireDate` 皆為 `null` → `titleName: null`、`isNewcomerAtWorkym(null, ...) = false`，自動滿足 AC-8 / BR-10，無需特殊分支。
...
**無**。`title_name`／`hire_date` 為 `ob_emphire` 既有欄位（T-7）；`workingDays` 為查詢時衍生值，不新增任何資料表或欄位。
...
- **回應體積**：新增 `projectWorkym`（6 字元字串）+ `workingDays`（數字）於 top-level（可忽略）；`titleName`/`isNewcomer` 僅新增於 `emplids[]` 層級（人力規模，數十至數百節點），非 `depts[]`/`byList` 逐名單重複，增量可忽略，遠低於 F055 OOM／getSummary 45s 案例之量級。
...
| 版本 | 日期 | 變更內容 |
|---|---|---|
| v1.1 | 2026-08-13 | **職稱來源欄位更正**（同日，隨母 spec F116 v1.1.1 反轉 BR-5）：使用者實機檢視 dev MSSQL 317 筆資料，發現 `ob_emphire.jfun_nm` 91%（289/317）為「營業一般職」——職能分類，於樞紐列標籤無辨識度；`ob_emphire.title_name` 才是真正職稱（業務專員 126／業務專員(電銷) 98／業務襄理 27／業務副理 15…等 13 種有意義分佈）。改為職稱來源**唯一** `ob_emphire.title_name`，**禁用** `jfun_nm`；API 欄位 `jfunNm` → `titleName`。同步修正 §2 事實 4、§4.2 SQL 示意（`.addSelect`/`.addGroupBy`）、§4.3 `isNewcomerAtWorkym` 周邊敘述、§4.4 response 契約、§5 mermaid 資料流圖、§6、§8 之全部欄位指涉。`title_name`（nvarchar(30)）與 `jfun_nm`（nvarchar(15)）皆為 `ob_emphire` 既有欄位、同為 nullable、NULL 筆數相同（各 12 筆）——**純同型別欄位替換**，`ROW_NUMBER() PARTITION BY emp_id` 去重 derived table 設計對來源欄寬度不敏感（去重鍵為 `emp_id`，與被選取的展示欄位無關），**不影響** A-1~A-5 任一裁定，不需新增/修改不變式 |
