...
資料集之間的遷移」。真正的「不可逆」風險在於：**業務一旦開始依賴 MSSQL 版之正式月跑結果做實際派案決策
...
graph TD
  ...
  P6c --> P6d[P6d 正式月跑 + F067 式對真實 legacy 比對驗收]
...
| **P6d** | 正式月跑一次，比照 F067 對真實 legacy SP 輸出做業務驗收 | P6c 完成 | 是（結果不滿意可重跑 P6c/P6d，尚未對外服務） |
...
**建議 P6e（正式上線）與 P6f（程式碼消除）之間保留一段觀察期（建議至少 1-2 個完整月跑週期），不在同一
批次內完成兩者。**
...
**若使用者傾向「一次做完、不留尾巴」**：可接受，但**強烈建議至少完成一次真實正式月跑並經業務簽核後
...
5. **P6d**：以 P6c 灌入之真實資料觸發一次正式月跑，比照 F067 既有方法論，將結果與 legacy SP 之真實輸出
   做業務級比對，產出簽核文件，**業務簽核**。
6. **P6e**：業務簽核通過後，正式對外/對內開放本系統，業務開始依賴月跑結果進行實際派案。**Point-of-no-return
   自此刻起算**（見 §5.4）。
7. **P6f**：觀察期滿（建議至少 1-2 個完整月跑週期，且期間無需回退至 PG 對照之情事）後，執行程式碼消除
   （§4.1）。
...
**定義：業務開始依據 MSSQL 版正式月跑結果，實際對外／對下游派案（即結果被業務流程真實採用、產生實際
...
| P6d 正式月跑 + F067 式驗收 | 2-3 人天 | 比照既有 F067/P5c/P5e 方法論，含報告撰寫 |
| P6e 上線 | 依業務排程 | 非工程工作量 |
...
| **I-MSSQL-CUTOVER-REVERSIBLE-01** | P6e（業務開始依賴 MSSQL 版正式月跑結果做實際派案決策）之前，P6 全部步驟必須維持可回退（改回環境變數即可還原，或資料庫可安全清空重建）；P6f（程式碼消除）不得早於 P6d 業務簽核通過 |
