...
| F004 | US-010 | E02 | 建立帳號 | P0-MVP | Admin 建立新帳號（姓名、Email、密碼、角色），角色支援全部 8 種 |
...
| F008 | US-014 | E02 | 指派／變更角色 | P0-MVP | 變更帳號角色（支援全部 8 種角色），保護最後一位 Admin |
| F045 | US-017 | E02 | 業務角色定義 | P0-MVP | 系統預設 8 種角色（2 系統 + 6 業務）Seed Data，`GET /api/roles` API |
...
| 自訂角色新增/刪除 | 8 種角色為系統預設 Seed Data，不開放 Admin 動態增減（US-017） | 不納入任何階段規劃 |
| Customer 360 角色存取控制 | 業務角色在 Customer 360 模組的欄位可見性與功能開關 | Phase 2（US-068） |
...
3. **Customer 360 角色存取控制（US-068）** — 業務角色的欄位可見性、遮罩規則與功能開關（原 Phase 3 合併至 Phase 2）
4. **資料同步排程** — 自動化資料抽取與同步
5. **進階資料來源類型** — 支援 API 端點、檔案上傳等
