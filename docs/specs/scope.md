...
| F004 | US-010 | E02 | 建立帳號 | P0-MVP | Admin 建立新帳號（姓名、Email、密碼、角色），角色支援 Admin / User 2 種 |
...
| F008 | US-014 | E02 | 指派／變更角色 | P0-MVP | 變更帳號角色（支援 Admin / User 2 種角色），保護最後一位 Admin |
| F045 | US-017 | E02 | 角色定義 | P0-MVP | 系統預設 2 種角色（Admin / User）Seed Data，`GET /api/roles` API |
...
| F117 | US-180 | E07 | 部門比例設定頁僅提供「有在職處長」之部門設定 | P1 | M03a UX 精煉（F079 之限縮）。✅ **v1.1 Approved**（2026-08-04 人工審閱閘）；OQ-F117-B1 裁決＝孤兒部門顯示鎖定＋後端保留、不做強制歸零 |
| F118 | US-181 | E07 | 從上月複製名單顯示「已複製過」提示 | P1 | M01 UX 精煉（F050 複製子流程）。✅ **v1.1 Approved**（2026-08-04 人工審閱閘）；判定採語意等價、複製範圍以實作為準修正三處 spec |

> **⚠️ 本對照表存在既有落差（2026-08-04 由 F117/F118 撰寫時發現，未於本輪回填）**：上表功能列止於 **F068**，而 `spec-index.md` 實際已登錄至 **F118**（F069~F116 共 48 個 feature 從未回填至本表）。本輪僅新增 F117 / F118 兩列以維持新功能之可追溯性，**刻意不回填** F069~F116——該回填工作量大且涉及既有優先級判定，屬獨立整理批次。同類落差亦存在於 `docs/stories/overview.md` 與 E07 `epic-brief.md`（US-166~US-181 未登錄），建議一併處理。權威來源請以 [spec-index.md](spec-index.md) 為準。
...
| 自訂角色新增/刪除 | 2 種角色為系統預設 Seed Data，不開放 Admin 動態增減（US-017） | 不納入任何階段規劃 |
...
| F002 User 登入 | 無 | F003 |
| F003 登出 | F001, F002 | 無 |
| F045 業務角色定義 | F001 | F004, F005, F008 |
| F004 建立帳號 | F001, F045 | F005, F006, F007, F008, F009, F010 |
