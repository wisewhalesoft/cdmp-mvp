...
> **v1.22（2026-08-18 / F119 / AD-E07-50 — 類別型篩選欄位文字比對運算子）**：`ob_list_definition.condition_payload.conditions[]` 之 categorical 條件項新增兩個 **optional** key：`operator`（`'in' | 'contains' | 'not_contains' | 'equals'`，缺漏 ≡ `'in'`）與 `keyword`（`string`，僅文字運算子適用，trim 後長度 1~100，落庫值為 trim 後之值）。**純加性擴充，無 migration，既有列不回填**（`ObListDefinitionConditionItem` interface 已具 index signature）。詳細欄位契約、NULL 語意矩陣、`caseyear` 排除文字運算子之限制、PG/MSSQL 兩方言等價保證，見 [`implementation-log/AD-E07-50-categorical-text-match-operators.md`](implementation-log/AD-E07-50-categorical-text-match-operators.md)。
>
...
用途：提供 F049 Stage 0 每日分派數量估算所需之工作日篩選（排除週末與假日）；並提供 F116 v1.1 樞紐分析「工作天」維度之月工作日數。
...
- **F116 v1.1 月工作日數（US-182 拍板，2026-08-13）**：`workingDays` = `rest_flg = '0'` 且 `calendar_date` 落於該 run `assignment_run.project_workym` 當月（該月 1 日 ~ 該月末日，含頭尾）之列數；沿用既有 `weekday` CalendarSource 判準（排除週末＋國定假日），與 F049 Stage 0 同源不分叉（F116 BR-12 / ⚠ A-2）。查詢區間須以 **`'YYYY-MM-DD'` 字串邊界**傳入，**不得**使用 JS `Date` 物件（UTC+8 換算會漏邊界日，F116 T-3）。**查無該月資料 → `workingDays = 0`，屬可容忍降級非錯誤**：端點仍回 200，由前端顯示 `-` 與提示，禁止 `NaN` / `Infinity` / 除以零（F116 BR-15 / AC-11）。

**相關功能**：[F049](features/F049-stage0-daily-estimate.md)、[F116 v1.1](features/F116-snapshot-pivot-analysis.md)（樞紐分析「工作天」維度月工作日數）
