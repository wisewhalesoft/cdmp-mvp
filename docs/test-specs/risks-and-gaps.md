...
### F036-RISK-001：customer_core 欄位總數規格標注「約 45」但實際加總為 49

- **來源**：US-049 目標表定義（A~H 分類欄位加總）
- **問題**：US-049 規格在 AC-6 與 columnCount 均標注「約 45 欄位」，但 A~H 分類實際欄位加總為 49（5+5+6+6+10+10+7+5）。此差異原因不明（可能來自業務評估後欄位合併或移除某些欄位）。若實作時 migration 以 49 欄建立但 API 回傳 `columnCount=45`，測試 TS-F036-003 將失敗。
- **影響**：TS-F036-003（columnCount 驗證）的精確數字不確定；測試目前以「約 45（允許微小差異）」描述，無法提供精確邊界
- **建議**：實作前向 Architecture 確認最終欄位清單；migration 完成後以實際欄位數更新 TS-F036-003、TS-F036-004、TS-F036-039 的預期結果
- **風險等級**：中（影響 3 個測試場景的精確預期值）
