...
| 月名單分派鎖驗證策略 | 分別植入 status='pending' 與 status='running' 的 assignment_run 紀錄，各確認回 409 |
...
| 項目 | 內容 |
|------|------|
| Given | CARD_TYPE='H'、active 版本；維度 ACCOUNT_AGE 有現有 score 區間 `[0,3]=10`、`[4,12]=20`；無月名單分派鎖；Sales Manager Token |
...
| 項目 | 內容 |
|------|------|
| Given | CARD_TYPE='H'、active 版本；無 column_name='CONTRACT_YEARS' 的維度；無月名單分派鎖；Sales Manager Token |
...
| 項目 | 內容 |
|------|------|
| Given | CARD_TYPE='H' 的 active 版本；維度 ACCOUNT_AGE 現存（status='active'）；無月名單分派鎖；Sales Manager Token |
...
|----|------|---------|---------|---------|------|---------|
| TS-F054-001 | 正常修改維度分數區間（覆寫式） | AC-2 | Integration | H active 版本；ACCOUNT_AGE 有 2 筆 score；無月名單分派鎖；SM Token | PUT /api/v1/assignment/scoring/dimensions（修改 ACCOUNT_AGE 2 筆區間） | HTTP 200；updatedScores=2；DB 舊 score 被替換；card_version 仍 1 |
...
| TS-F054-003 | 類別型（level1）區間修改正常 | AC-2, BR-3 | Integration | CELLULAR 維度有 2 筆 level1 類別型 score；無月名單分派鎖；SM Token | PUT 修改 level1 值與 score | HTTP 200；DB 更新正確；level2_s / level2_e 仍為 null |

### B. API Integration Tests — 新增維度

| ID | 場景 | 關聯需求 | 測試類型 | 前置條件 | 步驟 | 預期結果 |
|----|------|---------|---------|---------|------|---------|
| TS-F054-004 | 新增維度（數值型 2 個區間） | AC-3 | Integration | 無 CONTRACT_YEARS 維度；無月名單分派鎖；SM Token | POST /api/v1/assignment/scoring/dimensions（columnName='CONTRACT_YEARS'，2 個區間） | HTTP 201；DB: ob_levelcard_column 1 筆（status='active'）；ob_levelcard_score 2 筆 |
...
| ID | 場景 | 關聯需求 | 測試類型 | 前置條件 | 步驟 | 預期結果 |
|----|------|---------|---------|---------|------|---------|
| TS-F054-007 | 停用維度後 status='inactive' | AC-4 | Integration | ACCOUNT_AGE status='active'；無月名單分派鎖；SM Token | PUT /api/v1/assignment/scoring/dimensions/ACCOUNT_AGE/disable?cardType=H | HTTP 200；DB ob_levelcard_column.status='inactive'；ob_levelcard_score 原資料仍存在 |
...
### D. API Integration Tests — 月名單分派鎖與驗證
...
| TS-F054-013 | 數值型區間重疊（交集不為空）回 422 | AC-6, BR-3 | Integration | ACCOUNT_AGE 現有區間 [0,10]；無月名單分派鎖；SM Token | PUT 嘗試新增 [9,20] 區間 | HTTP 422，SCORING_RANGE_OVERLAP；DB 原有資料不變 |
| TS-F054-014 | 相鄰區間接觸（score_e+1=下一級 score_s）允許 | AC-6, BR-3 | Integration | ACCOUNT_AGE 現有區間 [0,10]；無月名單分派鎖；SM Token | PUT 新增 [11,20] 區間（相鄰接觸） | HTTP 200；不回傳 SCORING_RANGE_OVERLAP |
...
| ID | 場景 | 關聯需求 | 測試類型 | 前置條件 | 步驟 | 預期結果 |
|----|------|---------|---------|---------|------|---------|
| TS-F054-017 | 月名單分派執行中全部按鈕 DOM disabled（非 CSS 隱藏） | AC-5 | Frontend Unit | stub API 回傳月名單分派鎖定狀態（或由 assignment_run 狀態注入）；isLocked=true | 渲染計分設定頁 | 新增維度按鈕 disabled=true；編輯按鈕 disabled=true；停用按鈕 disabled=true；這些按鈕在 DOM 中存在但不可點擊 |
| TS-F054-018 | 月名單分派鎖定時頁面顯示提示訊息 | AC-5 | Frontend Unit | isLocked=true | 渲染計分設定頁 | 頁面顯示「分派執行中，無法修改計分設定」文字提示 |
...
-- 月名單分派鎖 fixture（用於 TS-F054-010 / 011 / 012）
-- pending 鎖：
...
| TS-F054-017 ~ 024（Frontend Unit） | 高 | React Testing Library；月名單分派鎖狀態以 mock context 注入 |
