...
- **Given** 月跑已觸發（status = 'running'）
- **When** 業務主管進入執行進度頁
- **Then** 顯示目前月跑的資訊：run_id、作業年月、觸發時間、目前狀態、已執行時間
...
- **Given** 月跑所有 Stage 均已完成（status = 'completed'）
- **When** 進度頁偵測到完成狀態
- **Then** 顯示「執行完成」提示，並提供快速連結「查看結果摘要」（跳轉至 US-083）與「匯出結果」（跳轉至 US-084）

### AC-4：月跑失敗顯示錯誤

- **Given** 月跑某 Stage 失敗（status = 'failed'）
- **When** 進度頁偵測到失敗狀態
...
- **相關 Stories**：US-081（觸發月跑）、US-083（結果摘要）、US-084（匯出結果）
