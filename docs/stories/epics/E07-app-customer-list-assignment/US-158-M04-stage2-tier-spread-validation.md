...
version: v1.0
change-summary: "新增 Story：Stage 2 計分修正後驗收——重跑 202606 月跑，驗證 card_level / tier 分佈出現合理 T1/T2/T3 spread，不再全部退化為 T3（最低 tier）"
---
...
**I want** 重跑 202606 月跑後，card_level 分佈出現多個等級（非全部 D），tier 分佈出現 T1/T2/T3 合理 spread
...
- **觸發方式**：透過 CDMP 月跑介面（M04）觸發，或直接呼叫月跑 API endpoint
- **查詢語句（參考）**：
  ```sql
