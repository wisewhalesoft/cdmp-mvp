# US-132：M01 Ready 欄頂 CTA Banner（月跑與試算唯一入口）
...
**I want** 在 Kanban 的「準備完成」欄頂部看到一個 CTA banner，包含「執行月跑」主按鈕與「試算」次要按鈕
**So that** 月跑的唯一觸發入口清晰可見，同時能在正式執行前先以試算確認預期結果，避免因入口分散造成操作混亂
...
- **v2.2**：移除 ready 階段各名單卡片上的 per-card「觸發」按鈕。月跑是月份級操作（F078 原子性月跑語意），per-list 觸發違反此語意。改以 Ready 欄頂 CTA banner 作為月跑唯一入口。
- **v2.3**：在 CTA banner 新增 secondary「試算」按鈕（連至 Stage 0 試算頁），與主按鈕並排。同步移除 Toolbar 上的「執行月跑」與「Stage 0 試算」重複入口按鈕。
...
- **And** banner 包含兩個並排按鈕：
  - 主按鈕（藍底白字）：「執行 YYYY-MM 月跑」，點擊導向 `31-trigger-run` 頁面
...
### AC-5：Toolbar 不再提供月跑或試算入口（重複入口移除）

- **Given** 部長或 Admin 在 M01 名單定義主頁
- **When** 頁面載入完成
- **Then** Toolbar 區域不顯示「執行月跑」按鈕，也不顯示「Stage 0 試算」按鈕
...
- 月跑執行中狀態判斷：查詢 AssignmentRun 是否有 status = 'running' 記錄（與 US-070 AC-4 邏輯一致）
- 主按鈕導向：`31-trigger-run.html`（對應 US-081 月跑觸發頁）
- secondary 按鈕導向：`30-stage0-estimate.html`（對應 US-071 Stage 0 試算頁）
- `data-write-action="trigger-month-run"` 屬性應掛在主按鈕上，歷史月份 / 月跑中時此 attribute 的元素自動隱藏或禁用
...
- **Given**：目前月份 2026-05；ready 欄有 3 份名單；無月跑執行中；部長帳號
- **When**：部長進入 M01 主頁
- **Then**：ready 欄頂部顯示綠色 banner「✓ 3 份名單已準備完成」；主按鈕「執行 2026-05 月跑」（藍底）；secondary「試算」（白底藍邊）
...
### TC-132-04：月跑執行中 Banner 禁用

- **Given**：AssignmentRun status = 'running'；ready 欄有 3 份名單
- **When**：Kanban 渲染
- **Then**：banner 呈禁用樣式（琥珀底色）；主按鈕與「試算」按鈕均 disabled

### TC-132-05：Toolbar 無月跑 / 試算按鈕

- **Given**：部長在 M01 主頁
- **When**：頁面載入
- **Then**：Toolbar 區域不存在「執行月跑」或「Stage 0 試算」任何按鈕

---

## 依賴關係

- **Blocked By**：US-130（Kanban 主頁，Banner 為 ready 欄的子元件）、US-081（月跑觸發頁，Banner 主按鈕導向目標）、US-071（Stage 0 試算頁，Banner secondary 按鈕導向目標）
- **Blocks**：（本 Story 為 UI 規則定義，不阻擋其他業務流程 Story）
- **取代**：v1 Toolbar 上的「執行月跑」與「Stage 0 試算」按鈕；ready 卡片上的 per-card 觸發按鈕
...
- **Gap 覆蓋**：G3（Ready CTA 雙按鈕 UI 規範）
- **相關 Stories**：US-130（Kanban 主頁）、US-081（月跑觸發，主按鈕目標）、US-071（Stage 0 試算，secondary 按鈕目標）、US-070（Toolbar 規則，AC-1 配合修改）
