...
version: v1.0
change-summary: "新增 story：F102 per-list cr_enabled 閘控——月跑依 ob_list_definition.cr_enabled 決定是否執行 CR 優先分派；cr_enabled=false 時全名單 is_cr 強制為 'N'，進入標準比例分派。"
---

# US-153：月跑 per-list CR 優先分派閘控（cr_enabled gate）
...
**As a** 業務主管
**I want** 月跑依照每份名單各自的 `ob_list_definition.cr_enabled` 設定，決定是否為該名單執行 CR 優先分派邏輯
**So that** 不同名單可有不同 CR 行為——啟用的名單走 CR 優先指派（US-152），停用的名單直接走標準比例分派，兩者可在同一次月跑中共存
...
- **Given** 名單 OB202606001 的 `ob_list_definition.cr_enabled = true`；月跑觸發
- **When** 月跑進入 Stage 3 前置 CR 處理
- **Then** 月跑為 OB202606001 執行 US-152 的步驟 1（逾2年清空）、步驟 2（離職清空）、步驟 3（CR 優先指派）
- **And** 月跑執行日誌記錄：`OB202606001：cr_enabled=true，執行 CR 優先分派`

### AC-2：cr_enabled=false 時跳過 CR 優先分派，is_cr 強制為 'N'

- **Given** 名單 OB202606002 的 `ob_list_definition.cr_enabled = false`
- **When** 月跑進入 Stage 3 前置 CR 處理
- **Then** 月跑跳過 OB202606002 的 US-152 步驟 1–3
- **And** OB202606002 所有案件的 `is_cr` 強制更新為 `'N'`（確保無殘留 'Y' 值）
- **And** OB202606002 所有案件進入 F101 標準比例分派池（不扣量）
- **And** 月跑執行日誌記錄：`OB202606002：cr_enabled=false，跳過 CR 優先分派`

### AC-3：同一月跑中多份名單 cr_enabled 可有不同設定

- **Given** 月跑包含 OB202606001（cr_enabled=true）與 OB202606002（cr_enabled=false）
- **When** 月跑完整執行（Stage 0–4）
- **Then** OB202606001 的 CR 案件有 `is_cr='Y'`、`emplid=cr_id`
- **And** OB202606002 的所有案件 `is_cr='N'`，走標準比例分派
- **And** 兩份名單的 Stage 3/4 比例分派互不干擾，月跑整體 status = 'completed'

### AC-4：cr_enabled 在月跑開始後鎖定，不受執行中修改影響

- **Given** 月跑開始時讀取 `ob_list_definition.cr_enabled` 快照
- **When** 月跑執行中（status = 'running'），管理員嘗試修改名單的 cr_enabled
- **Then** 修改操作被月跑鎖阻擋（已有 US-107 AC-5 / US-104 鎖定機制）
- **And** 月跑全程使用月跑開始時的 cr_enabled 快照值，不受後續變更影響
...
- **is_cr 強制清空（AC-2）**：cr_enabled=false 時，需在月跑工作集中執行 `UPDATE ob_monthly_run_result SET is_cr='N' WHERE list_no=:listNo AND is_cr IS NOT NULL AND is_cr<>'N'`，避免 ob_pool_data_list 中殘留的 is_cr='Y' 值污染月跑結果
- **不依賴全域旗標**：本 story 不讀取 `ob_assign_config.cr_reassignment_enabled`；全域旗標廢除由 US-154 負責
- **閘控讀取時機**：建議在月跑 Stage 3 前置處理的「per-list 迴圈」開始時讀取，與 F101 Stage 3 比例分派的 per-list 讀取點一致
...
- **[OPEN QUESTION-6]**：月跑開始時，是否需要在快照（AssignmentRunSnapshot）中記錄每份名單的 `cr_enabled` 值，以便事後稽核？目前 F101 快照含 ob_dept_pct/ob_empl_set 比例，是否需一併快照 cr_enabled？建議 spec-writer 確認 F066 快照範圍。
...
### TC-153-03：混合 cr_enabled 名單月跑不互相干擾

- **Given**：OB202606001 cr_enabled=true；OB202606002 cr_enabled=false；同一次月跑
- **When**：月跑完整執行
- **Then**：OB202606001 有 is_cr='Y' 案件；OB202606002 無 is_cr='Y' 案件；月跑 status='completed'
...
- [ ] cr_enabled=true/false 整合測試各覆蓋一次完整月跑（TC-153-03）
