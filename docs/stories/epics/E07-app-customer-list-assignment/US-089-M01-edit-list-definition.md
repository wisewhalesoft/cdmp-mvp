...
> **[DEPRECATED — 重構為 US-106 取代]**
> 本 Story 已由 **US-106（草稿階段建立名單 + 篩選條件）** 取代。
> 新架構下，名單篩選條件**只在草稿階段可編輯**；進入後續階段後條件鎖定，不再提供獨立的「編輯名單」功能。
> 原文內容保留作為歷史紀錄，**請勿基於本 Story 進行實作或 spec 撰寫**。
> 處置時間：2026-05-15（E07 重構 C+D 組）
