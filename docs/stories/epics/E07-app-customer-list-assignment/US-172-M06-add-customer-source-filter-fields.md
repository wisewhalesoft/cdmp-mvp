...
新增的 8 個客戶屬性篩選欄位均來自 `customer_core`，由月名單分派 Stage 1 以 LEFT JOIN 方式串接後過濾案件。當 LEFT JOIN 後客戶欄位為 NULL（即 `ob_pool_data` 中的案件在 `customer_core` 找不到對應客戶），該案件視為**不符合條件**，被排除出名單（與 INNER JOIN 過濾效果相同）。
...
- **Then** 可新增、停用、啟用該欄位的可選值，行為與現有 US-103 categorical 欄位完全一致（停用值不回溯既有名單、月名單分派不受影響）
...
- **And** 月名單分派 Stage 1 執行過濾時，以執行當下日期計算每位客戶的實足年齡（年齡 = 執行日期 − date_of_birth，以年為單位取整），再判斷是否落在 min–max 之間
  > **注意**：年齡計算基準日的精確語意（執行當天 vs 月名單分派作業月份首日）列為 OQ-172-01，見本文末尾
...
- **When** 月名單分派 Stage 1 執行該名單的案件過濾，以 `ob_pool_data.custo_no = customer_core.source_customer_no` LEFT JOIN 串接
...
- **When** 觸發月名單分派（US-081），月名單分派 Stage 1 讀取 `OB202607001` 的篩選條件
- **Then** 月名單分派仍正確讀取 JSONB 中固化的「性別 IN [1]」條件並依此過濾，不因欄位停用而失敗
...
- 月名單分派 Stage 1 的 LEFT JOIN SQL 實作語意（JOIN key、NULL COALESCE 策略等）由 spec-writer 及 tdd-implementation 負責
...
- 關聯鍵：`ob_pool_data.custo_no = customer_core.source_customer_no`；月名單分派以 LEFT JOIN 串接，NULL 語意見 AC-8
...
### TC-172-09：客戶欄位停用不中斷既有名單月名單分派

- **Given**：名單 `OB202607001` 篩選條件含「性別 IN [1]」；部長事後停用白名單「性別」欄位
- **When**：觸發月名單分派，月名單分派 Stage 1 讀取 `OB202607001` 的篩選條件
- **Then**：月名單分派仍以 JSONB 中固化的「性別 IN [1]」過濾 customer_core，月名單分派完成不報錯
...
- **Blocks**：月名單分派 Stage 1 實作需對應處理 LEFT JOIN 與 NULL 排除語意（US-081 相關）
...
| ID | 問題 | 負責方 | 狀態 |
|----|------|--------|------|
| OQ-172-01 | 年齡計算基準日語意：月名單分派 Stage 1 計算客戶年齡時，應以「月名單分派執行當天」還是「作業月份的第一天（project_workym 的月份第一日）」為基準？兩者在跨月邊界的客戶可能導致不同結果。 | 業務主管 | ✅ 已裁示 2026-07-01：**以作業月份首日（project_workym 月份第一日）為基準**。理由＝同一作業月重跑結果一致（年齡不隨執行日漂移），具決定性，供 F067 apples-to-apples 比對。 |
...
| OQ-172-04 | 若 `customer_core` ETL 尚未執行（`customer_core` 表為空），而名單定義含客戶篩選條件，月名單分派結果將為 0 筆（因所有案件 LEFT JOIN 後均為 NULL 排除）。是否需要在月名單分派執行前加入前置檢查，偵測 `customer_core` 無資料時給予業務警告？ | 業務主管 + system-architect | ✅ 已裁示 2026-07-01：**列為已知限制，不建前置檢查**（F109 範圍外）。理由＝dev/prod customer_core 皆有 360 萬筆，空表實務不會發生；保持範圍精簡，僅於 spec 註記此限制。 |
...
- **延伸 Stories**：US-102（篩選欄位白名單，需新增 `data_source` 支援）、US-103（類別型欄位可選值管理，客戶欄位可選值維護入口）、US-106（名單定義草稿篩選條件 UI，需來源分組）、US-081（月名單分派 Stage 1，需實作 LEFT JOIN customer_core 與 NULL 排除語意）
