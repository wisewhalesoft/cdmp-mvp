...
> **[DEPRECATED — 已由 US-109 完全取代]**
> 本 Story 原設計以全域 M03 入口設定部門比例，與重構後的五階段流程不符。
> **現行設計**：部門比例設定已整合至五階段流程第二階段（Stage 2 部門比例設定），由 **US-109**（部長/Admin 設定各部門分配比例）取代。
> **原文保留**：歷史參考用，請勿依本 Story 進行實作。spec-writer 需確保 F060（對應本 Story 的 spec）也標記為 DEPRECATED 並指向 F0XX（US-109 對應 spec）。
