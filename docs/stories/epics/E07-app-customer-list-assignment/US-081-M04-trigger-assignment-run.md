...
- **Given** 業務主管在分派執行頁面點擊「執行月名單分派」按鈕
- **When** 系統進行前置條件驗證
- **Then** 依序檢查以下條件：
  1. **所有本月 active 名單（LIST_NO）均需處於「準備完成（ready）」階段**（OQ-C-02 / US-118 決議：月名單分派只允許在所有 active 名單皆為 ready 時觸發；任一名單未達 ready 則阻擋月名單分派）
  ...
  6. 目前無 running 狀態的月名單分派（防止併發執行）
- **And** 若任一條件未滿足，顯示具體的失敗原因清單，不啟動月名單分派

> **[OQ-C-02 衍生]**：前置條件 1 的「所有 active 名單均需 ready 」由 US-118（準備完成查詢摘要）流程管理；業務主管可在 US-118 介面中確認所有名單狀態後，再觸發月名單分派。
...
- **And** 三份快照在同一 transaction 中寫入；任一快照寫入失敗則整體回滾，月名單分派標記為 'failed'

### AC-6：分數完整性 soft check（警告不阻擋）

- **Given** 月名單分派在 Stage 2（計分）開始前
- **When** 系統針對所有 active CARD_TYPE 執行分數完整性查核
- **Then** 系統檢查以下完整性規則，違規者**不阻擋月名單分派繼續執行**，但寫入 `assignment_audit_log`：
...
- **And** 月名單分派完成後的結果摘要頁（US-083）顯示「分數設定警告」區塊，列出所有違規維度，提示業務主管確認設定是否符合預期
- **And** 若當月無任何 active 維度有 score 設定（全部為空），仍執行月名單分派但 audit log 記錄 `SCORE_INTEGRITY_WARN` 等級最高警示

> **業務理由**：分數設定異常屬於計分參數問題，不是系統錯誤；阻擋月名單分派會造成業務流程中斷。soft check + audit log 讓業務主管在事後確認結果，而非事前被阻擋。

### AC-5：月名單分派失敗處理

- **Given** 月名單分派執行過程中某 Stage 發生錯誤
- **When** 錯誤被捕獲
...
- 月名單分派為非同步執行，前端透過輪詢或 WebSocket 取得進度（由 US-082 負責進度顯示）
- **執行觸發不受日期限制**：舊系統 SP 要求每月 21 日後才能執行的硬編碼限制已移除，業務主管可隨時觸發（A13）
- **允許整批覆蓋重跑**：同月可多次觸發月名單分派，每次產生新 run_id，前次結果保留於快照歷史（不刪除）；只阻擋併發執行（running 狀態中禁止再次觸發），不阻擋重跑（A5）
...
- **When**：業務主管點擊「執行月名單分派」
- **Then**：顯示錯誤清單「LIST_NO 'OB202605001' 的部門比例加總為 95%，需調整至 100%」，月名單分派不啟動
...
- **When**：業務主管再次點擊「執行月名單分派」並確認
- **Then**：前置條件通過（completed 狀態不阻擋重跑），系統建立新月名單分派並產生新 run_id（'new-002'）
- **And**：前次月名單分派快照（'prev-001'）完整保留於快照歷史，不被覆蓋或刪除

### TC-081-03b：併發執行防護

- **Given**：目前有 status = 'running' 的月名單分派
- **When**：業務主管嘗試再次點擊「執行月名單分派」
- **Then**：前置條件失敗，提示「分派執行中，請等待目前月名單分派完成後再觸發」，新月名單分派不啟動

### TC-081-05：分數完整性 soft check — 有違規但月名單分派繼續執行

- **Given**：CARD_TYPE = 'H' 的某維度 match_type = 'RANGE'，但 `ob_levelcard_score` 中無任何對應 score 列
- **When**：月名單分派啟動，Stage 2 開始前執行 soft check
- **Then**：月名單分派**繼續執行**（不阻擋）；`assignment_audit_log` 新增一筆 `event='SCORE_INTEGRITY_WARN'` 的記錄，包含 card_type='H'、rule_violated='NO_SCORE_ROWS'；US-083 結果摘要頁顯示「分數設定警告」區塊
...
- **Then**：月名單分派繼續；audit log 記錄 rule_violated='COMPOSITE_LEVEL2_OVERLAP'；結果摘要顯示警告

### TC-081-04：快照原子性（失敗回滾）

- **Given**：月名單分派執行成功，但寫入 result 快照時發生 DB 錯誤
- **When**：transaction 回滾
...
- **Blocked By**：**US-118**（準備完成階段：所有 active 名單需處於「準備完成」階段才可觸發月名單分派，OQ-C-02）、US-109（部門比例設定）、US-112（個別業務比例設定）、US-079（舊版人員比例設定，現由 US-112 取代）
...
- [ ] 單元測試覆蓋率 ≥ 80%
- [ ] 整合測試：完整月名單分派流程（Stage 1 ~ Stage 4）不拋出未捕獲例外
- [ ] 分數完整性 soft check 測試（TC-081-05、TC-081-06）：違規寫 audit log、月名單分派不阻擋
