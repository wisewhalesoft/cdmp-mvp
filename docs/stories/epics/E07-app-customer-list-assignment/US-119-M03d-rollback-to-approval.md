# US-119：準備完成階段 Rollback 至簽核（月名單分派前重新審核）
...
**So that** 在月名單分派前有最後一道退回機制，避免帶著錯誤設定執行月名單分派；退回後名單從準備完成清單移出
...
- 準備完成 → 簽核的 Rollback 用於「月名單分派前發現問題需重新審核」的場景
...
- US-118 的「月名單分派前置條件提示」（AC-4）依此 Story 的退回操作動態更新：名單退回後，提示重新計算未就緒名單
...
- **月名單分派關聯**：US-081（月名單分派觸發）依賴所有名單為 ready；退回後此名單不再計入 ready，月名單分派前提條件重新失效
...
- [ ] 退回後重新核准測試（TC-119-02）
- [ ] 處長被拒測試（TC-119-03）
- [ ] 月名單分派中禁止 Rollback 測試（TC-119-04）
- [ ] US-118 月名單分派前提示即時更新測試（TC-119-05）
...
- **相關 Stories**：US-105（五階段總覽，退回入口）、US-116（核准，Rollback 的逆操作）、US-117（拒絕，退回後可執行）、US-118（準備完成查詢，退回後移出清單）、US-081（月名單分派觸發，受退回影響的前置條件）

---

## v2.0 / v2.2 更新（2026-05-21）

> **變更來源**：v2.0 Kanban 重構（操作入口調整）+ v2.2 移除 per-card 月名單分派觸發
...
- **And** `ready` 卡片上**不存在**任何「觸發月名單分派」或「執行」相關按鈕（v2.2 移除，月名單分派唯一入口為 US-132 Ready CTA banner）
