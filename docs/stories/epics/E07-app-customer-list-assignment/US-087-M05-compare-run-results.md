...
- **Then** 頁面顯示兩次月名單分派的基本資訊並排（作業年月、觸發時間、總分派筆數）
- **And** 若兩次月名單分派的作業年月相同，顯示提示「同月比對通常用於重跑調參情境」
...
- **Then** 系統顯示「人員配對不一致」報告，包含：
  - 兩次月名單分派中**分派給不同業務員的案件清單**（每列顯示 APPL_NO、Base 月名單分派 OB_EMPLID、Compare 月名單分派 OB_EMPLID）
...
- **Given** 兩次月名單分派已選定
- **When** 業務主管查看「設定差異」區塊
- **Then** 列出兩次月名單分派的 config 快照差異項目，包含：
  - 計分版本是否變更（版本號）
  - 各部門比例是否有調整（差異 ≥ 1% 標示）
  - CR 回分規則是否有切換
...
- 比對邏輯基於兩次月名單分派的 AssignmentRunSnapshot（result 與 config 快照）
...
- **Blocked By**：US-086（兩次月名單分派的快照詳情需可讀取）
- **Blocks**：（無）
