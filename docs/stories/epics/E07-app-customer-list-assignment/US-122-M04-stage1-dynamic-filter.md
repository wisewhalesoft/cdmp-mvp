---
story-id: US-122
title: 月跑 Stage 1 動態 WHERE 條件執行（condition_payload 驅動）
epic: E07 — 客戶名單分派
module: M04 分派執行
priority: Must Have
status: Draft
...
# US-122：月跑 Stage 1 動態 WHERE 條件執行（condition_payload 驅動）
...
**I want** 觸發月跑後，系統依每份名單定義的篩選條件（condition_payload）動態過濾 OBPOOLDATA 案件池，而非撈取全表
...
現行月跑 Stage 1 採全表讀取 OBPOOLDATA（GAP-LIST D1 / D2）。v2.1 重構後，Stage 1 必須依 `condition_payload` 動態組合 WHERE 條件。
...
**backward-compat fallback**：condition_payload 為 NULL 的舊遷移名單，Stage 1 自動讀取 6 個 entity column，月跑不中斷（J6 / D4）。
...
- **Given** 月跑被觸發（US-081），Stage 1 開始讀取 `stage = 'ready'` 的名單列表
...
> **業務意義（A6/D1/D2）**：月跑結果精確反映每份名單的篩選邏輯，避免超出業務範圍的案件被錯誤納入分派。
...
- **And** fallback 路徑不報錯，月跑繼續正常執行完成
- **And** fallback 行為不修改名單資料（不觸發 condition_payload 寫入）

> **業務意義（D4/J6）**：保障遷移期間舊名單的月跑不中斷，業務主管無需手動重設舊名單條件。
...
- **When** 月跑 Stage 1 執行時讀取此名單
- **Then** 月跑仍正確依 condition_payload 中的 `settle_src` 條件過濾 OBPOOLDATA，不因欄位被停用而報錯或跳過該條件
- **And** 月跑讀取 condition_payload 時不查詢白名單的 is_active 狀態（條件已固化於儲存時）

> **業務意義**：停用欄位的「不回溯」語意延伸至月跑執行。已提交的名單條件不因管理員後續維護白名單而失效（與 US-102 AC-8 / US-103 AC-7 一致）。
...
- 月跑為非同步執行（AD-E07-2），Stage 1 的執行進度可由 US-082 查看
...
- **When**：月跑 Stage 1 執行此名單
- **Then**：Stage 1 以 entity column 組合 WHERE 條件，月跑完成不報錯；condition_payload 仍為 NULL（未被修改）

### TC-122-05：非 ready 名單不被月跑讀取

- **Given**：名單 `OB202507001` stage = 'draft'；名單 `OB202507002` stage = 'ready'
- **When**：月跑 Stage 1 執行
- **Then**：只讀取 `OB202507002`；`OB202507001` 不出現在 Stage 1 處理清單中

### TC-122-06：停用白名單欄位不影響既有名單月跑

- **Given**：名單條件含 `settle_src`；白名單 `settle_src` 被停用
- **When**：月跑 Stage 1 執行此名單
- **Then**：Stage 1 仍依 `settle_src` 條件過濾，月跑完成不報錯

---

## 依賴關係

- **Blocked By**：US-121（condition_payload 驗證規則與語意定義）、US-081（月跑觸發機制，Stage 1 為月跑的一部分）
- **Blocks**：月跑 Stage 2~4 邏輯（依賴 Stage 1 的案件輸出清單）
...
- **相關 Stories**：US-081（月跑觸發）、US-082（執行進度查看）、US-121（condition_payload 驗證規則）、US-123（舊名單 backward-compat 讀取）、US-102（白名單欄位停用不回溯語意）、US-103（可選值停用不回溯語意）
