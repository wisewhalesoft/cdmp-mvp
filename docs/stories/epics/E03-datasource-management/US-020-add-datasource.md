...
### AC-2：防止重複名稱
- **Given** 已存在一個名稱為「Production DB」且資料庫名稱為「prod_db」的資料來源
- **When** Admin 嘗試建立另一個名稱同為「Production DB」且資料庫名稱同為「prod_db」的資料來源
- **Then** 系統顯示「相同資料庫下已存在此名稱的資料來源」，且不建立該筆記錄

### AC-2b：不同資料庫允許相同名稱
- **Given** 已存在一個名稱為「Production DB」且資料庫名稱為「prod_db」的資料來源
- **When** Admin 建立另一個名稱同為「Production DB」但資料庫名稱為「staging_db」的資料來源
- **Then** 系統成功建立該資料來源，不顯示重複錯誤
...
| 4 | 使用相同名稱＋相同資料庫名稱新增 | 顯示重複錯誤訊息 |
| 4b | 使用相同名稱＋不同資料庫名稱新增 | 建立成功 |
| 5 | 缺少主機位址 | 驗證錯誤訊息 |
| 6 | 連接埠為非數字 | 驗證錯誤訊息 |
| 7 | 缺少密碼 | 驗證錯誤訊息 |
