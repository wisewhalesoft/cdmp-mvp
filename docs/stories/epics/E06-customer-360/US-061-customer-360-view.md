...
> **階段**：Phase 2
> **預估點數**：13
> **變更說明（2026-04-07）**：新增 AC-10（角色適應性頁面整合，依角色動態顯示功能區段）；AC-7 補充外籍客戶（'04'）視同個人客戶說明；Technical Notes 移除「Phase 2 暫行預設遮罩」，改為統一 Mask Middleware；API Response 欄位命名統一（移除 Masked 後綴）
...
### AC-10：角色適應性頁面整合（依角色動態顯示功能區段）
- **Given** 使用者登入並進入客戶 360 頁面
- **When** 頁面載入完成，系統識別使用者角色
- **Then** 頁面依角色顯示以下功能區段：
  - **基本資料（8 個分類）**：所有角色皆顯示，但各分類的欄位可見性依 US-068 `c360_field_visibility` 動態調整
  - **標籤管理入口（管理標籤按鈕）**：Admin、業務、行銷、客服、分析師、主管顯示；**後端作業角色不顯示**
  - **互動紀錄區段**：業務、客服（僅自己的紀錄）、主管（全部紀錄）顯示；其他角色不顯示
  - **變更歷史入口（頁籤/按鈕）**：客服、分析師、主管顯示；其他角色不顯示
  - **資料品質回報按鈕**：後端作業、分析師顯示；其他角色不顯示
...
- 敏感資料遮罩規則：遮罩邏輯由統一 Mask Middleware 提供，從 `c360_field_visibility`（US-068）讀取呼叫者角色的設定，動態決定各欄位分類是否完整顯示／遮罩／隱藏；**不使用 Phase 2 暫行預設遮罩**，所有遮罩行為均由 `c360_field_visibility` Seed Data 驅動
...
- [ ] 敏感資料遮罩邏輯由統一 Mask Middleware 依 US-068 c360_field_visibility 動態套用（無暫行預設遮罩）
- [ ] 前端頁面含 8 個資料分類展示（分類卡片或 Accordion）
- [ ] NULL 欄位顯示「—」處理完成
- [ ] 企業/個人客戶的適應性顯示邏輯實作完成
- [ ] 風控旗標高亮顯示實作完成
- [ ] ETL 資料新鮮度警告實作完成（超過 7 天）
- [ ] 標籤顯示與管理入口整合（接 US-064），後端作業角色不顯示標籤管理入口
- [ ] 角色適應性頁面整合：依角色動態顯示互動紀錄、變更歷史、資料品質回報等功能區段（AC-10）
- [ ] 頁面不存在時的 404 處理
- [ ] 效能符合 NFR-002（1 秒內載入）
- [ ] 單元測試覆蓋率達標（> 80%）
