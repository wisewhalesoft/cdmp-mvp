...
- **And** 敏感欄位（身分證/統編、電話）的遮罩規則：Admin 完整明碼顯示；User 與業務主管套用固定遮罩
...
- 權限：所有已登入身分（Admin / User / 業務主管）皆可存取客戶清單；業務主管為 role = `user` 且 is_sales_manager = true 的帳號
- 敏感資料遮罩：硬編碼於 API 層，依呼叫者身分決定：
  - Admin：所有欄位完整明碼
  - User / 業務主管：身分證前 3 碼 + 後 2 碼顯示（例：`A12****89`）；電話前 4 碼 + 後 2 碼顯示（例：`0912***78`）
...
| 7a | 業務主管查看清單中的 source_customer_no | 固定遮罩顯示 `A12****89`（同 User 遮罩規則） |
...
- [ ] 敏感資料遮罩硬編碼於 API 層（Admin 明碼 / User 與業務主管固定遮罩）
