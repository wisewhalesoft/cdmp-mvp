...
E06 開放以下角色存取，與 E02 帳號管理模組定義一致：

| 角色 | 說明 | 敏感資料存取 |
|------|------|-------------|
| Admin | 系統管理者，具備完整平台管理權限 | 完整明碼顯示 |
| User | 一般使用者 | 固定遮罩規則 |
| 業務主管（Sales Manager） | role = `user` 且 is_sales_manager = true 的帳號，可存取 E06 與 E07 全功能 | 固定遮罩規則（同 User） |

所有三種身分均可存取 E06 客戶搜尋與 360 檢視功能。業務主管的敏感資料遮罩規則與一般 User 相同。

### 敏感資料遮罩規則

- **Admin**：所有欄位完整明碼顯示，不遮罩
- **User / 業務主管**：敏感欄位套用固定遮罩規則，硬編碼於 API 層：
...
| # | 問題 | 決策結果 |
|---|------|---------|
| 1 | 角色定義 | Admin / User 兩種系統角色（2026-04-13 精簡）；業務主管為 User 子類旗標，同樣可存取 E06（2026-04-24 更新） |
