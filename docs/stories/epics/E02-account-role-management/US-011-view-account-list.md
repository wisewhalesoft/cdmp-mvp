...
> **階段**：Phase 1（MVP）
> **預估點數**：3
> **變更說明（2026-04-13）**：角色篩選從 8 種簡化為 2 種（Admin / User），移除六種業務角色相關 AC
...
- **When** 頁面載入完成
- **Then** 系統顯示分頁帳號清單，包含：姓名、Email、角色（顯示中文名稱：「管理者」或「使用者」）、狀態（啟用／停用）、建立日期
...
- `role` 篩選參數使用 role_code（`admin` 或 `user`）
- 搜尋為大小寫不敏感（SQL 使用 `ILIKE` 或 `LOWER()` 比對）
- Response：`{ data: [...], total, page, limit }`
- 回傳的帳號資料中，角色欄位以 `{ roleCode, displayName }` 格式呈現，前端顯示 `displayName`
- 預設排序：建立日期由新至舊
- 分頁：預設每頁 20 筆
...
| 3a | 依角色「User」篩選 | 僅顯示 User 帳號 |
| 4 | 依狀態「停用」篩選 | 僅顯示停用帳號 |
| 5 | 無符合結果 | 顯示空狀態訊息 |
| 6 | 翻頁操作 | 顯示正確頁面內容 |
| 7 | 帳號角色欄位顯示 | 角色正確顯示中文名稱（「管理者」或「使用者」） |
