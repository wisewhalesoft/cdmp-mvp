...
每次月跑完成時，系統必須寫入三份快照（設定快照 / 輸入名單快照 / 結果快照）至 AssignmentRunSnapshot 表。三份快照必須在同一 database transaction 中完成寫入，確保資料一致性——不允許出現「結果快照存在但設定快照缺失」等部分寫入狀態，以保障歷史追溯與稽核的完整性。
...
|----------|-----------|---------|
| US-081 | 觸發分派月跑 | 快照寫入的核心實作 |
| US-086 | 查看執行快照詳情 | 依賴快照完整性才能顯示三份分頁 |
| US-087 | 比對兩次執行結果差異 | 依賴兩次月跑各自的快照完整性 |
...
2. **整合測試**：執行完整月跑，驗證 completed 月跑恰好有三份對應快照
